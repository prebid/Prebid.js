...
function toStorageRequest(bid) {
  ...
  if (_config.config.getConfig('cache.vasttrack')) {
...
function store(bids, done) {
  let getAjax = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : _ajax.ajaxBuilder;
  const requestData = {
    puts: bids.map(toStorageRequest)
  };
  const ajax = getAjax(_config.config.getConfig('cache.timeout'));
  ajax(_config.config.getConfig('cache.url'), shimStorageCallback(done), JSON.stringify(requestData), {
    contentType: 'text/plain',
    withCredentials: true
  });
}
function getCacheUrl(id) {
  return "".concat(_config.config.getConfig('cache.url'), "?uuid=").concat(id);
}