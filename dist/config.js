...
exports.config = exports.RANDOM = void 0;
...
const DEFAULT_DEBUG = (0, _utils.getParameterByName)(_constants.default.DEBUG_MODE).toUpperCase() === 'TRUE';
...
const RANDOM = exports.RANDOM = 'random';
const FIXED = 'fixed';
const VALID_ORDERS = {};
...
function newConfig() {
  ...
  function resetConfig() {
    ...
    const props = {
      publisherDomain: {
        set(val) {
          if (val != null) {
            (0, _utils.logWarn)('publisherDomain is deprecated and has no effect since v7 - use pageUrl instead');
          }
      ...
      priceGranularity: {
        ...
        set(val) {
          if (validatePriceGranularity(val)) {
            ...
            } else if ((0, _utils.isPlainObject)(val)) {
              setProp('customPriceBucket', val);
              setProp('priceGranularity', GRANULARITY_OPTIONS.CUSTOM);
              (0, _utils.logMessage)('Using custom price granularity');
            }
          }
        }
      },
          ...
          val != null && setProp('mediaTypePriceGranularity', Object.keys(val).reduce((aggregate, item) => {
            if (validatePriceGranularity(val[item])) {
              if (typeof val === 'string') {
                ...
                (0, _utils.logMessage)("Using custom price granularity for ".concat(item));
              }
            } else {
              (0, _utils.logWarn)("Invalid price granularity for media type: ".concat(item));
            }
            return aggregate;
          }, {}));
        }
      ...
      bidderSequence: {
        ...
        set(val) {
          if (VALID_ORDERS[val]) {
            ...
            (0, _utils.logWarn)("Invalid order: ".concat(val, ". Bidder Sequence was not set."));
          }
        }
      },
      auctionOptions: {
    ...
    Object.defineProperties(newConfig, Object.fromEntries(Object.entries(props).map(_ref => {
      let [k, def] = _ref;
    ...
    function hasGranularity(val) {
      return (0, _polyfill.find)(Object.keys(GRANULARITY_OPTIONS), option => val === GRANULARITY_OPTIONS[option]);
    }
    function validatePriceGranularity(val) {
      if (!val) {
        (0, _utils.logError)('Prebid Error: no value passed to `setPriceGranularity()`');
        return false;
      }
      if (typeof val === 'string') {
        if (!hasGranularity(val)) {
          (0, _utils.logWarn)('Prebid Warning: setPriceGranularity was called with invalid setting, using `medium` as default.');
        }
      } else if ((0, _utils.isPlainObject)(val)) {
        if (!(0, _cpmBucketManager.isValidPriceConfig)(val)) {
          (0, _utils.logError)('Invalid custom price value passed to `setPriceGranularity()`');
          return false;
        }
      }
      return true;
    ...
    function validateauctionOptions(val) {
      if (!(0, _utils.isPlainObject)(val)) {
        (0, _utils.logWarn)('Auction Options must be an object');
        return false;
      }
      for (let k of Object.keys(val)) {
        if (k !== 'secondaryBidders' && k !== 'suppressStaleRender') {
          (0, _utils.logWarn)("Auction Options given an incorrect param: ".concat(k));
          return false;
        }
        if (k === 'secondaryBidders') {
          if (!(0, _utils.isArray)(val[k])) {
            (0, _utils.logWarn)("Auction Options ".concat(k, " must be of type Array"));
            return false;
          } else if (!val[k].every(_utils.isStr)) {
            (0, _utils.logWarn)("Auction Options ".concat(k, " must be only string"));
            return false;
          }
        } else if (k === 'suppressStaleRender') {
          if (!(0, _utils.isBoolean)(val[k])) {
            (0, _utils.logWarn)("Auction Options ".concat(k, " must be of type boolean"));
            return false;
          }
        }
      }
   ...
   */
  function _getConfig() {
    if (currBidder && bidderConfig && (0, _utils.isPlainObject)(bidderConfig[currBidder])) {
      ...
      return (0, _polyfill.arrayFrom)(configTopicSet).reduce((memo, topic) => {
        if (typeof currBidderConfig[topic] === 'undefined') {
          memo[topic] = config[topic];
        } else if (typeof config[topic] === 'undefined') {
          memo[topic] = currBidderConfig[topic];
        } else {
          if ((0, _utils.isPlainObject)(currBidderConfig[topic])) {
            memo[topic] = (0, _utils.mergeDeep)({}, config[topic], currBidderConfig[topic]);
          } else {
            memo[topic] = currBidderConfig[topic];
          }
        }
  ...
  const [getAnyConfig, getConfig] = [_getConfig, _getRestrictedConfig].map(accessor => {
    ...
    return function getConfig() {
      if (arguments.length <= 1 && typeof (arguments.length <= 0 ? undefined : arguments[0]) !== 'function') {
        ...
        return option ? (0, _utils.deepAccess)(accessor(), option) : _getConfig();
      }
  ...
  function setConfig(options) {
    if (!(0, _utils.isPlainObject)(options)) {
      (0, _utils.logError)('setConfig options must be an object');
      return;
    }
    ...
    topics.forEach(topic => {
      ...
      if ((0, _utils.isPlainObject)(defaults[topic]) && (0, _utils.isPlainObject)(option)) {
        option = Object.assign({}, defaults[topic], option);
      }
      try {
        topicalConfig[topic] = config[topic] = option;
      } catch (e) {
        (0, _utils.logWarn)("Cannot set config for property ".concat(topic, " : "), e);
      }
    });
  ...
  function setDefaults(options) {
    if (!(0, _utils.isPlainObject)(defaults)) {
      (0, _utils.logError)('defaults must be an object');
      return;
    }
  ...
  function subscribe(topic, listener) {
    ...
    if (typeof callback !== 'function') {
      (0, _utils.logError)('listener must be a function');
      return;
    }
    const nl = {
      topic,
  ...
  function callSubscribers(options) {
    ...
    listeners.filter(listener => (0, _polyfill.includes)(TOPICS, listener.topic)).forEach(listener => {
      listener.callback({
  ...
  function setBidderConfig(config) {
    ...
    try {
      ...
      config.bidders.forEach(bidder => {
        ...
        Object.keys(config.config).forEach(topic => {
          ...
          if ((0, _utils.isPlainObject)(option)) {
            const func = mergeFlag ? _utils.mergeDeep : Object.assign;
    ...
    function check(obj) {
      if (!(0, _utils.isPlainObject)(obj)) {
        throw 'setBidderConfig bidder options must be an object';
      }
      if (!(Array.isArray(obj.bidders) && obj.bidders.length)) {
        throw 'setBidderConfig bidder options must contain a bidders list with at least 1 bidder';
      }
      if (!(0, _utils.isPlainObject)(obj.config)) {
  ...
  function mergeConfig(obj) {
    if (!(0, _utils.isPlainObject)(obj)) {
      (0, _utils.logError)('mergeConfig input must be an object');
      return;
    }
    const mergedConfig = (0, _utils.mergeDeep)(_getConfig(), obj);
    setConfig({
      ...mergedConfig
    });
    return mergedConfig;
    ...
    return function (cb) {
      return function () {
        if (typeof cb === 'function') {
          ...
          (0, _utils.logWarn)('config.callbackWithBidder callback is not a function');
        }
      };
    };
  }
 ...
 */
const config = exports.config = newConfig();