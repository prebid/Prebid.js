...
exports.ttlCollection = ttlCollection;
...
function ttlCollection() {
  ...
  const callbacks = [];
  const pendingPurge = [];
  const markForPurge = monotonic ? entry => pendingPurge.push(entry) : entry => pendingPurge.splice((0, _utils.binarySearch)(pendingPurge, entry, el => el.expiry), 0, entry);
  let nextPurge, task;
  function reschedulePurge() {
    task && clearTimeout(task);
    if (pendingPurge.length > 0) {
      const now = (0, _utils.timestamp)();
  ...
  function mkEntry(item) {
    ...
    const [init, refresh] = Object.entries({
      ...
      return function () {
        ...
        _promise.GreedyPromise.resolve(getter(item)).then(val => {
