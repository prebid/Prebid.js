...
function getCpmStringValue(cpm, config, granularityMultiplier) {
  ...
  let bucketFloor = 0;
  let bucket = (0, _polyfill.find)(config.buckets, bucket => {
...
function isValidPriceConfig(config) {
  if ((0, _utils.isEmpty)(config) || !config.buckets || !Array.isArray(config.buckets)) {
    return false;
  }
  let isValid = true;
...
function getCpmTarget(cpm, bucket, granularityMultiplier) {
  ...
  let customRoundingFunction = _config.config.getConfig('cpmRoundingFunction');
  ...
  if (invalidRounding || typeof cpmTarget !== 'number') {
    (0, _utils.logWarn)('Invalid rounding function passed in config');
