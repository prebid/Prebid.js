...
exports.getHighestCpmBidsFromBidPool = exports.filters = exports.TARGETING_KEYS = void 0;
exports.isBidUsable = isBidUsable;
exports.newTargeting = newTargeting;
exports.sortByDealAndPriceBucketOrCpm = sortByDealAndPriceBucketOrCpm;
...
const TARGETING_KEYS = exports.TARGETING_KEYS = Object.keys(_constants.default.TARGETING_KEYS).map(key => _constants.default.TARGETING_KEYS[key]);

// return unexpired bids
const isBidNotExpired = bid => bid.responseTimestamp + (0, _bidTTL.getTTL)(bid) * 1000 > (0, _utils.timestamp)();

// return bids whose status is not set. Winning bids can only have a status of `rendered`.
const isUnusedBid = bid => bid && (bid.status && !(0, _polyfill.includes)([_constants.default.BID_STATUS.RENDERED], bid.status) || !bid.status);
let filters = exports.filters = {
  isActualBid(bid) {
    return bid.getStatusCode() === _constants.default.STATUS.GOOD;
  },
  isBidNotExpired,
  isUnusedBid
};
...
const getHighestCpmBidsFromBidPool = exports.getHighestCpmBidsFromBidPool = (0, _hook.hook)('sync', function (bidsReceived, highestCpmCallback) {
  ...
  if (!hasModified) {
    const bids = [];
    const dealPrioritization = _config.config.getConfig('sendBidsControl.dealPrioritization');
    // bucket by adUnitcode
    let buckets = (0, _utils.groupBy)(bidsReceived, 'adUnitCode');
    // filter top bid for each bucket by bidder
    Object.keys(buckets).forEach(bucketKey => {
      let bucketBids = [];
      let bidsByBidder = (0, _utils.groupBy)(buckets[bucketKey], 'bidderCode');
...
function newTargeting(auctionManager) {
  ...
  targeting.resetPresetTargeting = function (adUnitCode, customSlotMatching) {
    if ((0, _utils.isGptPubadsDefined)()) {
      ...
      const adUnits = auctionManager.getAdUnits().filter(adUnit => (0, _polyfill.includes)(adUnitCodes, adUnit.code));
      let unsetKeys = pbTargetingKeys.reduce((reducer, key) => {
        reducer[key] = null;
        return reducer;
      }, {});
      window.googletag.pubads().getSlots().forEach(slot => {
        let customSlotMatchingFunc = (0, _utils.isFn)(customSlotMatching) && customSlotMatching(slot);
        // reset only registered adunits
        adUnits.forEach(unit => {
          if (unit.code === slot.getAdUnitPath() || unit.code === slot.getSlotElementId() || (0, _utils.isFn)(customSlotMatchingFunc) && customSlotMatchingFunc(unit.code)) {
    ...
    adUnitCodes.forEach(function (unit) {
      ...
      if (astTag && astTag.keywords) {
        ...
        currentKeywords.forEach(key => {
          if (!(0, _polyfill.includes)(pbTargetingKeys, key.toLowerCase())) {
  ...
  function bidShouldBeAddedToTargeting(bid, adUnitCodes) {
    return bid.adserverTargeting && adUnitCodes && ((0, _utils.isArray)(adUnitCodes) && (0, _polyfill.includes)(adUnitCodes, bid.adUnitCode) || typeof adUnitCodes === 'string' && bid.adUnitCode === adUnitCodes);
  }
  ;
  ...
  function getDealBids(adUnitCodes, bidsReceived) {
    if (_config.config.getConfig('targetingControls.alwaysIncludeDeals') === true) {
      const standardKeys = true ? TARGETING_KEYS.concat(_native.NATIVE_TARGETING_KEYS) : TARGETING_KEYS.slice();

      // we only want the top bid from bidders who have multiple entries per ad unit code
      const bids = getHighestCpmBidsFromBidPool(bidsReceived, _reducers.getHighestCpm);
      ...
      return bids.map(bid => {
        if (bid.dealId && bidShouldBeAddedToTargeting(bid, adUnitCodes)) {
          return {
            [bid.adUnitCode]: getTargetingMap(bid, standardKeys.filter(key => typeof bid.adserverTargeting[key] !== 'undefined'))
          };
        }
  ...
  function getAllowedTargetingKeyValues(targeting, allowedKeys) {
    const defaultKeyring = Object.assign({}, _constants.default.TARGETING_KEYS, _constants.default.NATIVE_KEYS);
    const defaultKeys = Object.keys(defaultKeyring);
    const keyDispositions = {};
    (0, _utils.logInfo)("allowTargetingKeys - allowed keys [ ".concat(allowedKeys.map(k => defaultKeyring[k]).join(', '), " ]"));
    targeting.map(adUnit => {
      ...
      const keys = keyring.filter(kvPair => {
        ...
        const found = isCustom || (0, _polyfill.find)(allowedKeys, allowedKey => {
    ...
    (0, _utils.logInfo)("allowTargetingKeys - removed keys [ ".concat(removedKeys.join(', '), " ]"));
  ...
  targeting.getAllTargeting = function (adUnitCode) {
    let bidsReceived = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : getBidsReceived();
    const adUnitCodes = getAdUnitCodes(adUnitCode);

    // Get targeting for the winning bid. Add targeting for any bids that have
    // `alwaysUseBid=true`. If sending all bids is enabled, add targeting for losing bids.
    var targeting = getWinningBidTargeting(adUnitCodes, bidsReceived).concat(getCustomBidTargeting(adUnitCodes, bidsReceived)).concat(_config.config.getConfig('enableSendAllBids') ? getBidLandscapeTargeting(adUnitCodes, bidsReceived) : getDealBids(adUnitCodes, bidsReceived)).concat(getAdUnitTargeting(adUnitCodes));
    ...
    const defaultKeys = Object.keys(Object.assign({}, _constants.default.DEFAULT_TARGETING_KEYS, _constants.default.NATIVE_KEYS));
    let allowedKeys = _config.config.getConfig(CFG_ALLOW_TARGETING_KEYS);
    const addedKeys = _config.config.getConfig(CFG_ADD_TARGETING_KEYS);
    ...
    const auctionKeysThreshold = _config.config.getConfig('targetingControls.auctionKeyMaxChars');
    if (auctionKeysThreshold) {
      (0, _utils.logInfo)("Detected 'targetingControls.auctionKeyMaxChars' was active for this auction; set with a limit of ".concat(auctionKeysThreshold, " characters.  Running checks on auction keys..."));
  ...
  _config.config.getConfig('targetingControls', function (config) {
    if ((0, _utils.deepAccess)(config, CFG_ALLOW_TARGETING_KEYS) != null && (0, _utils.deepAccess)(config, CFG_ADD_TARGETING_KEYS) != null) {
      (0, _utils.logError)(TARGETING_KEY_CONFIGURATION_ERROR_MSG);
    }
  });
  ...
  function filterTargetingKeys(targeting, auctionKeysThreshold) {
    ...
    let targetingCopy = (0, _utils.deepClone)(targeting);
    ...
    return targetingMap.reduce(function (accMap, currMap, index, arr) {
      ...
      if (querySize <= auctionKeysThreshold) {
        ...
        (0, _utils.logInfo)("AdUnit '".concat(code, "' auction keys comprised of ").concat(querySize, " characters.  Deducted from running threshold; new limit is ").concat(auctionKeysThreshold), targetingCopy[code]);
        accMap[code] = targetingCopy[code];
      } else {
        (0, _utils.logWarn)("The following keys for adUnitCode '".concat(code, "' exceeded the current limit of the 'auctionKeyMaxChars' setting.\nThe key-set size was ").concat(querySize, ", the current allotted amount was ").concat(auctionKeysThreshold, ".\n"), targetingCopy[code]);
      }
      if (index + 1 === arr.length && Object.keys(accMap).length === 0) {
        (0, _utils.logError)('No auction targeting keys were permitted due to the setting in setConfig(targetingControls.auctionKeyMaxChars).  Please review setup and consider adjusting.');
      }
      return accMap;
    }, {});
  }
  ...
  targeting.setTargetingForGPT = function (targetingConfig, customSlotMatching) {
    window.googletag.pubads().getSlots().forEach(slot => {
      Object.keys(targetingConfig).filter(customSlotMatching ? customSlotMatching(slot) : (0, _utils.isAdUnitCodeMatchingSlot)(slot)).forEach(targetId => {
        ...
        (0, _utils.logMessage)("Attempting to set targeting-map for slot: ".concat(slot.getSlotElementId(), " with targeting-map:"), targetingConfig[targetId]);
  ...
  function getAdUnitCodes(adUnitCode) {
    if (typeof adUnitCode === 'string') {
      return [adUnitCode];
    } else if ((0, _utils.isArray)(adUnitCode)) {
      return adUnitCode;
    }
  ...
  function getBidsReceived() {
    ...
    if (!_config.config.getConfig('useBidCache')) {
      ...
      const filterFunction = _config.config.getConfig('bidCacheFilterFunction');
      if (typeof filterFunction === 'function') {
        bidsReceived = bidsReceived.filter(bid => latestAuctionForAdUnit[bid.adUnitCode] === bid.auctionId || !!filterFunction(bid));
      }
    }
    bidsReceived = bidsReceived.filter(bid => (0, _utils.deepAccess)(bid, 'video.context') !== _mediaTypes.ADPOD).filter(isBidUsable);
    bidsReceived.forEach(bid => {
      bid.latestTargetedAuctionId = latestAuctionForAdUnit[bid.adUnitCode];
      return bid;
    });
    return getHighestCpmBidsFromBidPool(bidsReceived, _reducers.getOldestHighestCpmBid);
  }
  ...
  targeting.getWinningBids = function (adUnitCode) {
    let bidsReceived = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : getBidsReceived();
    const adUnitCodes = getAdUnitCodes(adUnitCode);
    return bidsReceived.filter(bid => (0, _polyfill.includes)(adUnitCodes, bid.adUnitCode)).filter(bid => _bidderSettings.bidderSettings.get(bid.bidderCode, 'allowZeroCpmBids') === true ? bid.cpm >= 0 : bid.cpm > 0).map(bid => bid.adUnitCode).filter(_utils.uniques).map(adUnitCode => bidsReceived.filter(bid => bid.adUnitCode === adUnitCode ? bid : null).reduce(_reducers.getHighestCpm));
  };
  ...
  targeting.setTargetingForAst = function (adUnitCodes) {
    ...
    try {
      ...
      (0, _utils.logError)('unable to reset targeting for AST' + e);
    }
    Object.keys(astTargeting).forEach(targetId => Object.keys(astTargeting[targetId]).forEach(key => {
      (0, _utils.logMessage)("Attempting to set targeting for targetId: ".concat(targetId, " key: ").concat(key, " value: ").concat(astTargeting[targetId][key]));
      // setKeywords supports string and array as value
      if ((0, _utils.isStr)(astTargeting[targetId][key]) || (0, _utils.isArray)(astTargeting[targetId][key])) {
        let keywordsObj = {};
        let regex = /pt[0-9]/;
    ...
    winners = winners.map(winner => {
      return {
        [winner.adUnitCode]: Object.keys(winner.adserverTargeting).filter(key => typeof winner.sendStandardTargeting === 'undefined' || winner.sendStandardTargeting || standardKeys.indexOf(key) === -1).reduce((acc, key) => {
          ...
          if (key === _constants.default.TARGETING_KEYS.DEAL) {
  ...
  function getStandardKeys() {
    ...
    .map(targeting => targeting.key).concat(TARGETING_KEYS).filter(_utils.uniques); // standard keys defined in the library.
  }
  ...
  function mergeAdServerTargeting(acc, bid, index, arr) {
    function concatTargetingValue(key) {
      return function (currentBidElement) {
        if (!(0, _utils.isArray)(currentBidElement.adserverTargeting[key])) {
  ...
  function getCustomKeys() {
    let standardKeys = getStandardKeys();
    if (true) {
      standardKeys = standardKeys.concat(_native.NATIVE_TARGETING_KEYS);
    }
    return function (key) {
  ...
  function getCustomBidTargeting(adUnitCodes, bidsReceived) {
    return bidsReceived.filter(bid => (0, _polyfill.includes)(adUnitCodes, bid.adUnitCode)).map(bid => Object.assign({}, bid)).reduce(mergeAdServerTargeting, []).map(truncateCustomKeys).filter(bid => bid); // removes empty elements in array;
  }
  ...
  function getBidLandscapeTargeting(adUnitCodes, bidsReceived) {
    const standardKeys = true ? TARGETING_KEYS.concat(_native.NATIVE_TARGETING_KEYS) : TARGETING_KEYS.slice();
    const adUnitBidLimit = _config.config.getConfig('sendBidsControl.bidLimit');
    const bids = getHighestCpmBidsFromBidPool(bidsReceived, _reducers.getHighestCpm, adUnitBidLimit);
    const allowSendAllBidsTargetingKeys = _config.config.getConfig('targetingControls.allowSendAllBidsTargetingKeys');
    const allowedSendAllBidTargeting = allowSendAllBidsTargetingKeys ? allowSendAllBidsTargetingKeys.map(key => _constants.default.TARGETING_KEYS[key]) : standardKeys;

    // populate targeting keys for the remaining bids
    return bids.map(bid => {
      if (bidShouldBeAddedToTargeting(bid, adUnitCodes)) {
        return {
          [bid.adUnitCode]: getTargetingMap(bid, standardKeys.filter(key => typeof bid.adserverTargeting[key] !== 'undefined' && allowedSendAllBidTargeting.indexOf(key) !== -1))
        };
      }
    }).filter(bid => bid); // removes empty elements in array
  }
  ...
  function getAdUnitTargeting(adUnitCodes) {
    function getTargetingObj(adUnit) {
      return (0, _utils.deepAccess)(adUnit, _constants.default.JSON_MAPPING.ADSERVER_TARGETING);
    }
    function getTargetingValues(adUnit) {
      const aut = getTargetingObj(adUnit);
      return Object.keys(aut).map(function (key) {
        if ((0, _utils.isStr)(aut[key])) aut[key] = aut[key].split(',').map(s => s.trim());
        if (!(0, _utils.isArray)(aut[key])) aut[key] = [aut[key]];
        return {
          [key]: aut[key]
        };
      });
    }
    return auctionManager.getAdUnits().filter(adUnit => (0, _polyfill.includes)(adUnitCodes, adUnit.code) && getTargetingObj(adUnit)).map(adUnit => {
      return {
        [adUnit.code]: getTargetingValues(adUnit)
      };
    });
  }
  targeting.isApntagDefined = function () {
    if (window.apntag && (0, _utils.isFn)(window.apntag.setKeywords)) {
      return true;
    }
  };
  return targeting;
}
const targeting = exports.targeting = newTargeting(_auctionManager.auctionManager);