...
function isValidParamsHost(params) {
	...
	return (
		params && params.host && typeof params.host === 'string' && params.host.split('.').length === 2
	);
}
 ...
 * @param options
 */
export function createUserSyncGetter(
	options = {
		iframeSyncUrl: '',
		imageSyncUrl: ''
	}
) {
	return function getUserSyncs(
		syncOptions,
		responses,
		gdprConsent = {},
		uspConsent = '',
		...
		const cidArr = responses
			.filter((resp) => resp?.body?.cid)
			.map((resp) => resp.body.cid)
			.filter(uniques);
		let params = `?cid=${encodeURIComponent(cidArr.join(','))}&gdpr=${
			gdprApplies ? 1 : 0
		}&gdpr_consent=${encodeURIComponent(consentString || '')}&us_privacy=${encodeURIComponent(
			uspConsent || ''
		)}&coppa=${encodeURIComponent(coppa)}`;
...
export function buildRequestData(
	...
	const contentLang =
		bidderRequest?.ortb2?.site?.content?.language || document.documentElement.lang;
...
export function createBuildRequestsFn(
	createRequestDomain,
	createUniqueRequestData,
	storage,
	bidderCode,
	bidderVersion,
	allowSingleRequest
	...
	function buildSingleRequest(bidRequests, bidderRequest, topWindowUrl, bidderTimeout) {
		...
		let chSize = 10;
		if (
			config.getConfig(`${bidderCode}.chunkSize`) &&
			typeof config.getConfig(`${bidderCode}.chunkSize`) === 'number'
		) {
	...
	return function buildRequests(validBidRequests, bidderRequest) {
		...
		if (singleRequestMode) {
			...
			const bannerBidRequests = validBidRequests.filter((bid) =>
				isArray(bid.mediaTypes)
					? bid.mediaTypes.includes(BANNER)
					: bid.mediaTypes[BANNER] !== undefined
			);
			if (bannerBidRequests.length > 0) {
				const singleRequests = buildSingleRequest(
					bannerBidRequests,
					bidderRequest,
					topWindowUrl,
					bidderTimeout
			...
			const videoBidRequests = validBidRequests.filter(
				(bid) => bid.mediaTypes[VIDEO] !== undefined
			);
