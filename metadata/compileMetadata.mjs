...
function validatePurposes({component, gvlid}, {legIntPurposes, purposes, flexiblePurposes, specialFeatures}) {
  RELEVANT_PURPOSES.forEach(purpose => {
    if (legIntPurposes.includes(purpose) && purposes.includes(purpose)) {
      logPurposeMsg(purposeErrors, component, gvlid, {purposes, legIntPurposes}, `declares both consent and LI as legal basis for purpose ${purpose}`)
    }
    if (flexiblePurposes.includes(purpose) && (!purposes.includes(purpose) && !legIntPurposes.includes(purpose))) {
      logPurposeMsg(purposeErrors, component, gvlid, {purposes, legIntPurposes, flexiblePurposes}, `declares purpose ${purpose} as flexible, but no legal basis for it`)
    }
...
export default async function compileMetadata(fetch = true) {
  ...
  if (purposeErrors.length > 0) {
    ...
    throw new Errors('Some purpose declarations are out of spec')
  }
  logErrorSummary();
