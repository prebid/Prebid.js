...
export function isValidIdentity(identity) {
  return !!(
    ...
    identity.refresh_from &&
    identity.refresh_token &&
    identity.refresh_expires
  );
}
        ...
        success: (responseText) => {
          try {
            if (!refreshDetails.refresh_response_key) {
              this._logInfo(
                'No response decryption key available, assuming unencrypted JSON'
              );
              ...
              window.crypto.subtle
                .importKey(
                  ...
                  this.createArrayBuffer(
                    atob(refreshDetails.refresh_response_key)
                  ),
                  { name: 'AES-GCM' },
                  false,
                  ['decrypt']
                  ...
                  (key) => {
                    ...
                    window.crypto.subtle
                      ...
                      .then(
                        ...
                        (reason) =>
                          this._logWarn(`Call to UID2 API failed`, reason)
                      );
                  },
...
export class Uid2StorageManager {
  ...
  readCookie(cookieName) {
    return this._storage.cookiesAreEnabled()
      ? this._storage.getCookie(cookieName)
      : null;
  }
  readLocalStorage(key) {
    return this._storage.localStorageIsEnabled()
      ? this._storage.getDataFromLocalStorage(key)
      : null;
  }
  readModuleCookie() {
  ...
  writeModuleStorage(value) {
    this._storage.setDataInLocalStorage(
      this._storageName,
      JSON.stringify(value)
    );
  }
  ...
  getStoredValueWithFallback() {
    const preferredStorageLabel = this._preferLocalStorage
      ? 'local storage'
      : 'cookie';
    const preferredStorageGet = (
      this._preferLocalStorage ? this.readModuleStorage : this.readModuleCookie
    ).bind(this);
    const preferredStorageSet = (
      this._preferLocalStorage
        ? this.writeModuleStorage
        : this.writeModuleCookie
    ).bind(this);
    const fallbackStorageGet = (
      this._preferLocalStorage ? this.readModuleCookie : this.readModuleStorage
    ).bind(this);
    ...
    if (!storedValue) {
      ...
      if (fallbackValue) {
        this._logInfo(
          `${preferredStorageLabel} was empty, but found a fallback value.`
        );
        if (typeof fallbackValue === 'object') {
          this._logInfo(
            `Copying the fallback value to ${preferredStorageLabel}.`
          );
      ...
      if (fallbackValue && typeof fallbackValue === 'object') {
        this._logInfo(
          `${preferredStorageLabel} contained a basic token, but found a refreshable token fallback. Copying the fallback value to ${preferredStorageLabel}.`
        );
        preferredStorageSet(fallbackValue);
        return fallbackValue;
      }
...
export function Uid2GetId(config, prebidStorageManager, _logInfo, _logWarn) {
  ...
  if (suppliedToken && storedTokens) {
    if (
      ...
      _logInfo(
        'Server supplied new token - ignoring stored value.',
        storedTokens.originalToken?.advertising_token,
        suppliedToken.advertising_token
      );
  ...
  if (FEATURES.UID2_CSTG) {
    const cstgIdentity = clientSideTokenGenerator.getValidIdentity(
      config.cstg,
      _logWarn
    );
    if (cstgIdentity) {
      if (
        storedTokens &&
        clientSideTokenGenerator.isStoredTokenInvalid(
          cstgIdentity,
          storedTokens,
          _logInfo,
          _logWarn
      ...
      if (
        !storedTokens ||
        Date.now() > storedTokens.latestToken.refresh_expires
      ) {
        const promise = clientSideTokenGenerator.generateTokenAndStore(
          config.apiBaseUrl,
          config.cstg,
          cstgIdentity,
          storageManager,
        ...
        return {
          callback: (cb) => {
            promise.then((result) => {
              _logInfo(
                'Token generation responded, passing the new token on.',
                result
              );
              cb(result);
            });
  ...
  const useSuppliedToken =
    !storedTokens?.latestToken ||
    (suppliedToken &&
      suppliedToken.identity_expires >
        storedTokens.latestToken.identity_expires);
  const newestAvailableToken = useSuppliedToken
    ? suppliedToken
    : storedTokens.latestToken;
  _logInfo(
    'UID2 module selected latest token',
    useSuppliedToken,
  ...
  if (Date.now() > newestAvailableToken.identity_expires) {
    const promise = refreshTokenAndStore(
      config.apiBaseUrl,
      newestAvailableToken,
      config.clientId,
      storageManager,
