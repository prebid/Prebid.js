$$PREBID_GLOBAL$$.module('express', function ({
    pbjs,
    utils,
    config
}) {
    ...
    if (!config.enabled) {
        utils.logWarn('config.enabled not set, not loading ' + this.name);
        return;
    }
    utils.logMessage('loading ' + this.name);

    var adUnits = config.adUnits || pbjs.adUnits || [];

    if(adUnits.length === 0) {
        utils.logWarn('no valid adUnits found, not loading ' + this.name);
    }
    ...
    window.googletag.cmd.push(function () {
        ...
        var pads = gpt.pubads;
        if (!gpt.display || !gpt.enableServices || typeof pads !== 'function' ||
            !pads().refresh || !pads().disableInitialLoad || !pads().getSlots || !pads().enableSingleRequest) {
        ...
        function mapGptSlotSizes(aGPTSlotSizes) {
            ...
            for (var i = 0; i < aGPTSlotSizes.length; i++) {
                try {
                    ...
                    utils.logWarn('slot size ' + aGPTSlotSizes[i].toString() + ' not supported by' + this.name);
                }
            }
            return aSlotSizes;
        }
