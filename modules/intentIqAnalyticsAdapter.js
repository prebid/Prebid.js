...
let iiqAnalyticsAnalyticsAdapter = Object.assign(adapter({ url: defaultUrl, analyticsType }), {
...
function initLsValues() {
  ...
  const fpid = readData(FIRST_PARTY_KEY)
  if (fpid) initOptions.fpid = JSON.parse(fpid)
  ...
  if (iiqArr[0].params && iiqArr[0].params.partner && !isNaN(iiqArr[0].params.partner)) {
    ...
    initOptions.userPercentage = iiqArr[0].params.percentage;

    initOptions.currentGroup = readData(GROUP_LS_KEY + '_' + initOptions.partner);
    initOptions.currentPercentage = readData(PERCENT_LS_KEY + '_' + initOptions.partner);
  }
}
...
function initReadLsIds() {
  ...
  try {
    ...
    let iData = readData(FIRST_PARTY_KEY + '_' + initOptions.partner);
    if (iData) {
...
function bidWon(args) {
  ...
  if (initOptions.lsValueInitialized && !initOptions.lsIdsInitialized) {
    initReadLsIds();
  }
  if (!initOptions.manualReport) {
    ajax(constructFulllUrl(preparePayload(args)), undefined, null, { method: 'GET' });
  }
  initOptions = {};
...
function preparePayload(data) {
  ...
  if (initOptions.userGroup && initOptions.userGroup != '') {
    result[PARAMS_NAMES.ABTestingConfigurationSource] = 'group';
  }

  if (initOptions.userPercentage && !isNaN(initOptions.userPercentage)) {
...
function fillEidsData(result) {
  if (initOptions.lsIdsInitialized) {
    ...
    result[PARAMS_NAMES.auctionEidsLegth] = initOptions.eidl;
  }
}
...
function constructFulllUrl(data) {
  ...
  return (
    defaultUrl +
        ...
        initOptions.partner +
        '&mct=1' +
        (initOptions?.fpid?.pcid
          ? '&iiqid=' + encodeURIComponent(initOptions.fpid.pcid)
          : '') +
        '&agid=' +
        REPORTER_ID +
        '&jsver=' +
