...
const ENDPOINT = 'https://localhost:8050/request-bid'
...
export const spec = {
  ...
  buildRequests: function (validBidRequests, bidderRequest) {
    ...
    return {
      ...
      url: `${ENDPOINT}`,
      data: {
        bidRequests: validBidRequests.map(({ params, sizes }) => ({
          params,
          sizes
        }))
      },
  ...
  interpretResponse: function (serverResponse, originalBidRequest) {
    ...
    originalBidRequest.validBidRequests.forEach(function (bidRequest) {
      ...
      const matchedBid = find(serverResponse.body.bids, function (bid) {
        return String(bidRequest.params.placementId) === String(bid.id)
      })
      ...
      if (matchedBid) {
        ...
        const bidResponse = {
          ...
          creativeId: String(matchedBid.id), // ??
          dealId: String(matchedBid.id), // ??
