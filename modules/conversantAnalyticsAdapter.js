...
function onBidWon(args) {
  ...
  if (!bidderCode || !adUnitCode || !auctionId) {
    throw new Error('onBidWon() did not get all the necessary data to process the event: bidderCode = ' + bidderCode + ', adUnitCode = ' + adUnitCode + ', auctionId = ' + auctionId);
  }
...
function onAdRenderFailed(args) {
  ...
  if (!adId || !cnvrHelper.adIdLookup[adId]) {
    let errorMsg = 'No ad id found';
    if (adId) {
      errorMsg = 'No data found in lookup for given ad id.';
    }
    // Either no adId to match against a bidWon event, or no data saved from a bidWon event that matches the adId
    throw new Error('onAdRenderFailed(): Unable to process RENDER FAILED because ' + errorMsg);
  }
  ...
  if (!bidderCode || !adUnitCode || !auctionId) {
    throw new Error('onAdRenderFailed() did not get all the necessary data to process the event: bidderCode = ' + bidderCode + ', adUnitCode = ' + adUnitCode + ', auctionId = ' + auctionId);
  }
...
function onAuctionEnd(args) {
  const auctionId = args.auctionId;
  if (!auctionId) {
    throw new Error('onAuctionEnd(): No auctionId supplied.');
  }
  ...
  if (!Array.isArray(args.adUnits)) {
    throw new Error('onAuctionEnd(): No adUnits supplied in event args');
  }
