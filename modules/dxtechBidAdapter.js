...
export const spec = {
  ...
  buildRequests: function (validBidRequests, bidderRequest) {
    ...
    let baseEndpoint = spec.ENDPOINT + '?publisher_id=' + publisherId;

    if (placementId) {
      baseEndpoint += '&placement_id=' + placementId
    }
  ...
  getUserSyncs: function (syncOptions, serverResponses, gdprConsent, uspConsent) {
    ...
    logInfo('dxtech.getUserSyncs result=%o', syncs);
    return syncs;
  },
...
function outstreamRenderer(bid) {
  ...
  try {
    ...
    logWarn('dxtech: Prebid Error calling setRender on renderer', err);
  }
...
function _validateParams(bidRequest) {
  ...
  if (!bidRequest.params.publisherId) {
    logError('dxtech: Validation failed: publisherId not declared');
    return false;
  }

  if (!bidRequest.params.placementId) {
    logError('dxtech: Validation failed: placementId not declared');
    return false;
  }
...
function _validateVideo(bidRequest) {
  ...
  if (!Array.isArray(videoParams.mimes) || videoParams.mimes.length === 0) {
    logError('dxtech: Validation failed: mimes are invalid');
    return false;
  }

  if (!Array.isArray(videoParams.protocols) || videoParams.protocols.length === 0) {
    logError('dxtech: Validation failed: protocols are invalid');
    return false;
  }

  if (!videoParams.context) {
    logError('dxtech: Validation failed: context id not declared');
    return false;
  }

  if (videoParams.context !== 'instream') {
    logError('dxtech: Validation failed: only context instream is supported ');
    return false;
  }

  if (typeof videoParams.playerSize === 'undefined' || !Array.isArray(videoParams.playerSize) || !Array.isArray(videoParams.playerSize[0])) {
    logError('dxtech: Validation failed: player size not declared or is not in format [[w,h]]');
    return false;
  }
