...
function getExt(firstPartyData) {
  const ext = {
    ...
    pbjs: "$prebid.version$",
    keywords: firstPartyData.site?.keywords
      ? firstPartyData.site.keywords.split(",").map((k) => k.trim())
      : [],
  };
  return {
    ttdprebid: ext,
...
function getUser(bidderRequest) {
  ...
  var eids = utils.deepAccess(bidderRequest, "userIdAsEids");
  if (eids.length) {
...
function video(bid) {
  ...
  const playbackmethod = utils.deepAccess(
    bid,
...
export const spec = {
  ...
  isBidRequestValid: function (bid) {
    ...
    if (!bid.params.supplySourceId) {
      utils.logWarn(
        BIDDER_CODE + ": Missing required parameter params.supplySourceId"
      );
      return false;
    }
    if (!alphaRegex.test(bid.params.supplySourceId)) {
      utils.logWarn(
        BIDDER_CODE + ": supplySourceId must only contain alphabetic characters"
      );
      return false;
    }
    if (!bid.params.publisherId) {
      utils.logWarn(
        BIDDER_CODE + ": Missing required parameter params.publisherId"
      );
      return false;
    }
    if (bid.params.publisherId.length > 32) {
      utils.logWarn(
        BIDDER_CODE + ": params.publisherId must be 32 characters or less"
      );
      return false;
    }
    ...
    if (mediaTypesVideo) {
      if (
        !mediaTypesVideo.maxduration ||
        !utils.isInteger(mediaTypesVideo.maxduration)
      ) {
        utils.logWarn(
          BIDDER_CODE +
  ...
  interpretResponse: function (response, serverRequest) {
    ...
    seatBidsInResponse.forEach((seatBid) => {
      seatBid.bid.forEach((bid) => {
        let matchingRequestedImpression = requestedImpressions.find(
          (imp) => imp.id === bid.impid
        );
  ...
  getUserSyncs: function (
    ...
    let url = `${USER_SYNC_ENDPOINT}/track/usersync?us_privacy=${encodeURIComponent(
      uspConsent
    )}${gdprParams}`;
