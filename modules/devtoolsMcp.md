...
When the build runs in the browser, the module listens for Chrome's `devtoolstooldiscovery` event and responds with a `ToolGroup` named `Prebid.js DevTools`.
...
Returns Prebid event history with event type, event id, elapsed time, sequence number, and sanitized event args.
