...
// ***** SMARTADS ADAPTER *****
export const BIDDER_CODE = 'smartads'
...
export const spec = {
    ...
    buildRequests: (validBidRequests = [], bidderRequest) => { // FORMAT REQUEST TO BID SERVER
        validBidRequests = convertOrtbRequestToProprietaryNative(validBidRequests)

        // ***** GET FIRST PARTY DATA CONFIG (SPECIFICLY FOR SMARTADS) *****
        const siteConfig_FPD = config.getConfig("site") || {};
        const userConfig_FPD = config.getConfig("user") || {};

        const winTop = getWindowTop();
        const device = getDevice();
        const imp = [];
        const fENDPOINT = ENDPOINT + (siteConfig_FPD.fosp_uid || "")
        const request = {
            ...
            imp,
            device,
            myvne_id: userConfig_FPD.myvne_id || "",
            orig_aid: siteConfig_FPD.orig_aid,
            fosp_aid: siteConfig_FPD.fosp_aid,
            fosp_uid: siteConfig_FPD.fosp_uid,
            id: siteConfig_FPD.id,
        }

        console.log('request.bidderRequest.ortb2 @@@@', bidderRequest.ortb2);
