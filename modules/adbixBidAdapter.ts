...
export const spec = {
  ...
  isBidRequestValid: function (bid: any) {
    ...
    return !!(params.publisherId && params.placementId && bid.mediaTypes && bid.mediaTypes.banner && Array.isArray(bid.mediaTypes.banner.sizes));
  },
  buildRequests: function (validBidRequests: any[], bidderRequest: any) {
    ...
    const request = {
      ...
      imp: validBidRequests.map((bid: any) => {
        const params = getAdbixParams(bid);
        const sizes = bid.mediaTypes.banner.sizes.map((size: number[]) => ({ w: Number(size[0]), h: Number(size[1]) }));
        return {
          id: bid.bidId,
          banner: { format: sizes },
    ...
    (body.seatbid || []).forEach((seatbid: any) => {
      (seatbid.bid || []).forEach((bid: any) => {
        ...
        bids.push({
          ...
          netRevenue: true,
          ad: bid.adm,
          meta: { advertiserDomains: Array.isArray(bid.adomain) ? bid.adomain : [] }
        });
      });
    });
    return bids;
