...
const BB_CONSTANTS = {
  ...
  AUCTION_URL:
    "$$URL_STARTpbs.bluebillywig.com/openrtb2/auction?pub=$$PUBLICATION",
  SYNC_URL:
    "$$URL_STARTpbs.bluebillywig.com/static/cookie-sync.html?pub=$$PUBLICATION",
...
const BB_HELPERS = {
  addSiteAppDevice: function (request, pageUrl) {
    if (typeof getConfig("app") === "object") request.app = getConfig("app");
      ...
      request.site = {};
      if (typeof getConfig("site") === "object")
        request.site = getConfig("site");
      if (pageUrl) request.site.page = pageUrl;
    }

    if (typeof getConfig("device") === "object")
      request.device = getConfig("device");
  ...
  substituteUrl: function (url, publication, renderer) {
    return url
      .replace("$$URL_START", DEV_MODE ? "https://dev." : "https://")
  ...
  getRendererUrl: function (publication, renderer) {
    return BB_HELPERS.substituteUrl(
      BB_CONSTANTS.RENDERER_URL,
      publication,
      renderer
    );
  },
  transformVideoParams: function (videoParams, videoParamsExt) {
    ...
    let playerSize = videoParams.playerSize || [
      BB_CONSTANTS.DEFAULT_WIDTH,
      BB_CONSTANTS.DEFAULT_HEIGHT,
    ];
    ...
    if (videoParamsExt)
      videoParams = Object.assign(videoParams, videoParamsExt);

    const videoParamsProperties = Object.keys(videoParams);

    videoParamsProperties.forEach((property) => {
      if (BB_CONSTANTS.VIDEO_PARAMS.indexOf(property) === -1)
        delete videoParams[property];
    });
  ...
  transformRTBToPrebidProps: function (bid, serverResponse) {
    ...
    if (
      extPrebidCache &&
      typeof extPrebidCache.vastXml === "object" &&
      extPrebidCache.vastXml.cacheId &&
      extPrebidCache.vastXml.url
    ) {
    ...
    if (!bidObject.vastUrl && bid.nurl && !bid.adm) {
...
const BB_RENDERER = {
  bootstrapPlayer: function (bid) {
    ...
    if (!bid.vastXml && !bid.vastUrl) {
      logWarn(
        `${BB_CONSTANTS.BIDDER_CODE}: No vastXml or vastUrl on bid, bailing...`
      );
      return;
    }
    ...
    if (!(window.bluebillywig && window.bluebillywig.renderers)) {
      logWarn(
        `${BB_CONSTANTS.BIDDER_CODE}: renderer code failed to initialize...`
      );
      return;
    }

    const rendererId = BB_RENDERER.getRendererId(
      bid.publicationName,
      bid.rendererCode
    );
    ...
    if (renderer) renderer.bootstrap(config, ele, bid.rendererSettings || {});
      ...
      logWarn(
        `${BB_CONSTANTS.BIDDER_CODE}: Couldn't find a renderer with ${rendererId}`
      );
  },
  newRenderer: function (rendererUrl, adUnitCode) {
    ...
    try {
      ...
      logWarn(
        `${BB_CONSTANTS.BIDDER_CODE}: Error tying to setRender on renderer`,
        err
      );
    }
...
export const spec = {
  ...
  isBidRequestValid(bid) {
    ...
    if (!bid.params) {
      logError(
        `${BB_CONSTANTS.BIDDER_CODE}: no params set on bid. Rejecting bid: `,
        bid
      );
      return false;
    }
    ...
    if (
      !bid.params.hasOwnProperty("publicationName") ||
      typeof bid.params.publicationName !== "string"
    ) {
      logError(
        `${BB_CONSTANTS.BIDDER_CODE}: no publicationName specified in bid params, or it's not a string. Rejecting bid: `,
        bid
      );
      return false;
    } else if (!publicationNameRegex.test(bid.params.publicationName)) {
      logError(
        `${BB_CONSTANTS.BIDDER_CODE}: publicationName must be in format 'publication' or 'publication.environment'. Rejecting bid: `,
        bid
      );
      return false;
    }
    ...
    if (
      !bid.params.hasOwnProperty("rendererCode") ||
      typeof bid.params.rendererCode !== "string"
    ) {
      logError(
        `${BB_CONSTANTS.BIDDER_CODE}: no rendererCode was specified in bid params. Rejecting bid: `,
        bid
      );
      return false;
    } else if (!rendererRegex.test(bid.params.rendererCode)) {
      logError(
        `${BB_CONSTANTS.BIDDER_CODE}: rendererCode must be alphanumeric, including underscores. Rejecting bid: `,
        bid
      );
      return false;
    }
    ...
    if (!bid.params.accountId) {
      logError(
        `${BB_CONSTANTS.BIDDER_CODE}: no accountId specified in bid params. Rejecting bid: `,
        bid
      );
      return false;
    }
    ...
    if (bid.params.hasOwnProperty("connections")) {
      if (!Array.isArray(bid.params.connections)) {
        logError(
          `${BB_CONSTANTS.BIDDER_CODE}: connections is not of type array. Rejecting bid: `,
          bid
        );
        return false;
      } else {
        for (let i = 0; i < bid.params.connections.length; i++) {
          if (!bid.params.hasOwnProperty(bid.params.connections[i])) {
            logError(
              `${BB_CONSTANTS.BIDDER_CODE}: connection specified in params.connections, but not configured in params. Rejecting bid: `,
              bid
            );
            return false;
          }
      ...
      logError(
        `${BB_CONSTANTS.BIDDER_CODE}: no connections specified in bid. Rejecting bid: `,
        bid
      );
      return false;
    }
    ...
    if (
      bid.params.hasOwnProperty("video") &&
      (bid.params.video === null || typeof bid.params.video !== "object")
    ) {
      logError(
        `${BB_CONSTANTS.BIDDER_CODE}: params.video must be of type object. Rejecting bid: `,
        bid
      );
      return false;
    }
    ...
    if (
      bid.params.hasOwnProperty("rendererSettings") &&
      (bid.params.rendererSettings === null ||
        typeof bid.params.rendererSettings !== "object")
    ) {
      logError(
        `${BB_CONSTANTS.BIDDER_CODE}: params.rendererSettings must be of type object. Rejecting bid: `,
        bid
      );
      return false;
    }
    ...
    if (
      ...
      if (!bid.mediaTypes[VIDEO].hasOwnProperty("context")) {
        logError(
          `${BB_CONSTANTS.BIDDER_CODE}: no context specified in bid. Rejecting bid: `,
          bid
        );
        return false;
      }
      ...
      if (bid.mediaTypes[VIDEO].context !== "outstream") {
        logError(
          `${BB_CONSTANTS.BIDDER_CODE}: video.context is invalid, must be "outstream". Rejecting bid: `,
          bid
        );
        return false;
      }
      ...
      logError(
        `${BB_CONSTANTS.BIDDER_CODE}: mediaTypes or mediaTypes.video is not specified. Rejecting bid: `,
        bid
      );
      return false;
    }
  ...
  buildRequests(validBidRequests, bidderRequest) {
    ...
    validBidRequests.forEach((validBidRequest) => {
      if (!this.syncStore.publicationName)
        this.syncStore.publicationName = validBidRequest.params.publicationName;
      if (!this.syncStore.accountId)
        this.syncStore.accountId = validBidRequest.params.accountId;

      const ext = validBidRequest.params.connections.reduce(
        (extBuilder, connection) => {
          extBuilder[connection] = validBidRequest.params[connection];

          if (this.syncStore.bidders.indexOf(connection) === -1)
            this.syncStore.bidders.push(connection);
      ...
      const videoParams = BB_HELPERS.transformVideoParams(
        deepAccess(validBidRequest, "mediaTypes.video"),
        deepAccess(validBidRequest, "params.video")
      );
      imps.push({
    ...
    BB_HELPERS.addSiteAppDevice(
      request,
      bidderRequest.refererInfo && bidderRequest.refererInfo.page
    );
  ...
  interpretResponse(serverResponse, request) {
    ...
    if (
      !serverResponse.hasOwnProperty("seatbid") ||
      !Array.isArray(serverResponse.seatbid)
    ) {
      return [];
    }
    ...
    serverResponse.seatbid.forEach((seatbid) => {
      ...
      seatbid.bid.forEach((bid) => {
        ...
        const bidParams = find(
          request.bidderRequest.bids,
          (bidderRequestBid) => bidderRequestBid.bidId === bid.bidId
        ).params;
        ...
        const rendererUrl = BB_HELPERS.getRendererUrl(
          bid.publicationName,
          bid.rendererCode
        );
  ...
  getUserSyncs(syncOptions, serverResponses, gdpr) {
    ...
    if (gdpr.gdprApplies && gdpr.consentString)
      queryString.push(`gdpr_consent=${gdpr.consentString}`);

    if (this.syncStore.uspConsent)
      queryString.push(`usp_consent=${this.syncStore.uspConsent}`);

    queryString.push(`accountId=${this.syncStore.accountId}`);
    queryString.push(`bidders=${btoa(JSON.stringify(this.syncStore.bidders))}`);
    queryString.push(
      `cb=${Date.now()}-${Math.random().toString().replace(".", "")}`
    );

    if (DEV_MODE) queryString.push("bbpbs_debug=true");

    // NB syncUrl by default starts with ?pub=$$PUBLICATION
    const syncUrl = `${BB_HELPERS.getSyncUrl(
      this.syncStore.publicationName
    )}&${queryString.join("&")}`;
