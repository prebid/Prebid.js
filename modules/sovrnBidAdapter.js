...
const ORTB_VIDEO_PARAMS = {
  ...
  protocols: (value) =>
    Array.isArray(value) && value.every((v) => v >= 1 && v <= 10),
  ...
  battr: (value) =>
    Array.isArray(value) && value.every((v) => v >= 1 && v <= 17),
  maxextended: (value) => isInteger(value),
  minbitrate: (value) => isInteger(value),
  maxbitrate: (value) => isInteger(value),
  boxingallowed: (value) => [0, 1].indexOf(value) !== -1,
  playbackmethod: (value) =>
    Array.isArray(value) && value.every((v) => v >= 1 && v <= 6),
  playbackend: (value) => [1, 2, 3].indexOf(value) !== -1,
  delivery: (value) =>
    Array.isArray(value) && value.every((v) => v >= 1 && v <= 3),
...
export const spec = {
  ...
  isBidRequestValid: function (bid) {
    ...
    return !!(
      ...
      (!video ||
        Object.keys(REQUIRED_VIDEO_PARAMS).every((key) =>
          REQUIRED_VIDEO_PARAMS[key](video[key])
        ))
    );
  },
  ...
  buildRequests: function (bidReqs, bidderRequest) {
    try {
      ...
      _each(bidReqs, function (bid) {
        ...
        const floorInfo =
          bid.getFloor && typeof bid.getFloor === "function"
            ? bid.getFloor({
                currency: "USD",
                mediaType:
                  bid.mediaTypes && bid.mediaTypes.banner ? "banner" : "video",
                size: "*",
              })
            : {};
        floorInfo.floor =
          floorInfo.floor || getBidIdParameter("bidfloor", bid.params);
        ...
        if (deepAccess(bid, "mediaTypes.banner")) {
          ...
          bidSizes =
            isArray(bidSizes) && isArray(bidSizes[0]) ? bidSizes : [bidSizes];
          bidSizes = bidSizes.filter((size) => isArray(size));
          const processedSizes = bidSizes.map((size) => ({
        ...
        .map((seat) =>
          seat.bid.map((sovrnBid) => {
            const bid = {
              ...
              meta: {
                advertiserDomains:
                  sovrnBid && sovrnBid.adomain ? sovrnBid.adomain : [],
              },
            };

            if (sovrnBid.nurl) {
              bid.ad = decodeURIComponent(
                `${sovrnBid.adm}<img src="${sovrnBid.nurl}">`
              );
            } else {
    ...
    try {
      ...
      if (serverResponses && serverResponses.length !== 0) {
        if (syncOptions.iframeEnabled) {
          const iidArr = serverResponses
            .filter((resp) => deepAccess(resp, "body.ext.iid"))
        ...
        if (syncOptions.pixelEnabled) {
          serverResponses
            .filter((resp) => deepAccess(resp, "body.ext.sync.pixels"))
...
function _buildVideoRequestObj(bid) {
  ...
  if (bidSizes) {
    ...
    const sizes = Array.isArray(videoAdUnitParams.playerSize[0])
  ...
  Object.keys(ORTB_VIDEO_PARAMS).forEach((paramName) => {
    if (videoParams.hasOwnProperty(paramName)) {
      if (ORTB_VIDEO_PARAMS[paramName](videoParams[paramName])) {
        ...
        logWarn(
          `The OpenRTB video param ${paramName} has been skipped due to misformating. Please refer to OpenRTB 2.5 spec.`
        );
      }
    }
  });
