...
const URL = 'http://preuse.reforge.in/bid?rtb_seat_id=0011&secret_key=wzP8eKAVkc&type=prebid&appid=40';
export const spec = {
  code: BIDDER_CODE,
...
function createOutstreamScript(bid) {
  ...
  utils.logMessage('[reforge][renderer] Handle reforge outstream renderer');
...
function outstreamRender(bid) {
  ...
  if (bid.renderer.config.outstream_function != null && typeof bid.renderer.config.outstream_function === 'function') {
    ...
    try {
      ...
      utils.logError('[reforge][renderer] Error:' + err.message)
    }
  }
}
registerBidder(spec);
