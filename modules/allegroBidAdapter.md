...
|----------------------------------|----------|---------|-----------------------------------------------------------------------------|---------------------------------------------------------|
| `allegro.bidderUrl`              | optional | String  | Custom bidder endpoint URL                                                  | `https://prebid.rtb.allegro.com/v1/rtb/prebid/bid` |
...
var adUnits = [{
  ...
  bids: [{
    bidder: 'allegro',
    params: {
      publisherId: 'gwp'   // publisher identifier; maps to site.publisher.id in the outgoing OpenRTB request
    }
  }]
}];
```
...
|----------|----------|--------|-------------------------------------------------------------------------------------------------------------------------|
| `publisherId` | optional | String | Publisher identifier. When present, the value is written to `site.publisher.id` in the outgoing OpenRTB bid request, enabling publisher-scoped bid entity filtering on the server side. |
