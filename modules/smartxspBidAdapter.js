...
const DEFAULT_BID_HOST = 'tag.smartxsp.io';
const DEFAULT_CURRENCY = 'EUR';
const DEFAULT_PROTOCOL = 'https';
const DEFAULT_TTL = 600;
const SMARTXSP_ANTENNA = 'tag.smartxsp.io';
const SMARTXSP_VERSION = '1.0.0';
...
export function log(msg, obj) {
  logInfo('SmartXSPBidAdapter - ' + msg, obj);
}
...
/**
 * Get a notifyId from bid params or from smartxsp global
 * @param {Object} params - The bid params
 * @return {string}
 */
function getNotifyId(params) {
  const smartxsp = window.smartxsp = window.smartxsp || {};

  let notifyId = params.notifyId || smartxsp.notifyId;
  if (!notifyId) {
    notifyId = generateUUID();
    log('generating a notifyId', notifyId);
  }
  if (!smartxsp.notifyId) {
    smartxsp.notifyId = notifyId;
  }
...
function isBidRequestValid(bid) {
  ...
  if (notifyId !== window.smartxsp.notifyId) {
    log(`notifyId mismatch: params [${bid.params.notifyId}] / smartxsp [${window.smartxsp.notifyId}]`);
    return false;
  }
