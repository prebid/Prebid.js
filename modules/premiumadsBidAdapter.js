import {
  logWarn, isArray, inIframe, isNumber, isStr, deepClone, deepSetValue, logError, isBoolean
} from '../src/utils.js';
...
const LOG_WARN_PREFIX = 'PremiumAds: ';
...
function _parseAdSlot(bid) {
  ...
  if (bid.hasOwnProperty('mediaTypes') && bid.mediaTypes.hasOwnProperty(BANNER) && bid.mediaTypes.banner.hasOwnProperty('sizes')) {
    var i = 0;
    var sizeArray = [];
...
function _initConf(refererInfo) {
  return {
    pageURL: refererInfo?.page || window.location.href, refURL: refererInfo?.ref || window.document.referrer
  };
}
...
function _createOrtbTemplate(conf) {
  return {
    ...
    imp: [],
    site: {
      page: conf.pageURL, ref: conf.refURL, publisher: {}
    },
    device: {
      ip: '',
      ...
      geo: {
        country: '{country_code}', type: 0, ipservice: 1, region: '', city: '',
      },
    },
    user: {},
    ext: {}
  ...
  if (sizes !== UNDEFINED && isArray(sizes)) {
    ...
    if (!bid.params.width && !bid.params.height) {
      if (sizes.length === 0) {
        ...
        bannerObj = UNDEFINED;
        logWarn(LOG_WARN_PREFIX + 'Error: mediaTypes.banner.size missing for adUnitId: ' + bid.params.adUnitId + '. Ignoring the banner impression in the adunit.');
        return bannerObj;
      } else {
...
function _createVideoRequest(bid) {
  ...
  if (videoData !== UNDEFINED) {
    ...
    videoObj = UNDEFINED;
    logWarn(LOG_WARN_PREFIX + 'Error: Video config params missing for adunit: ' + bid.params.adUnitId + ' with mediaType set as video. Ignoring video impression in the adunit.');
  }
  return videoObj;
}
...
function _createImpressionObject(bid, conf) {
  ...
  if (bid.hasOwnProperty('mediaTypes')) {
    ...
    bannerObj = {
      pos: 0, w: bid.params.width, h: bid.params.height, topframe: inIframe() ? 0 : 1
    };
  ...
  return impObj.hasOwnProperty(BANNER) || impObj.hasOwnProperty(NATIVE) || impObj.hasOwnProperty(VIDEO) ? impObj : UNDEFINED;
}
...
export const spec = {
  ...
  isBidRequestValid: function (bid) {
    if (bid && bid.params) {
      if (!bid.params.adUnitId) {
        logWarn(LOG_WARN_PREFIX + 'Error: missing adUnitId');
        return false;
      }
      if (!isNumber(bid.params.adUnitId)) {
        logWarn(LOG_WARN_PREFIX + 'Error: adUnitId is must be numeric');
        return false;
      }
      return true;
    }
  ...
  buildRequests: function (validBidRequests, bidderRequest) {
    ...
    return {
      method: 'POST', url: ENDPOINT_URL, data: JSON.stringify(payload), bidderRequests: bidderRequest
    };
  },
  interpretResponse: function (serverResponses, bidderRequest) {
    const bidResponses = [];
    var respCur = CURRENCY;
    ...
    try {
      if (serverResponses.body && serverResponses.body.seatbid && isArray(serverResponses.body.seatbid)) {
        ...
        serverResponses.body.seatbid.forEach(seatbidder => {
          seatbidder.bid && isArray(seatbidder.bid) && seatbidder.bid.forEach(bid => {
            let newBid = {
              requestId: bid.impid,
  ...
  getUserSyncs: function (syncOptions, responses, gdprConsent, uspConsent) {
    ...
    return responses.reduce((accum, rsp) => {
      let cookieSyncs = JSON.parse(rsp.body.customdata);
      if (cookieSyncs) {
