...
function isBidResponseValid(bid) {
  ...
  switch (bid.mediaType) {
    ...
    case NATIVE:
      return Boolean(
        bid.native &&
          bid.native.impressionTrackers &&
          bid.native.impressionTrackers.length
      );
    default:
      return false;
  }
...
export const spec = {
  ...
  isBidRequestValid: (bid = {}) => {
    ...
    let valid = Boolean(
      bidId && params && (params.placementId || params.endpointId)
    );
    ...
    if (mediaTypes && mediaTypes[BANNER]) {
      ...
      valid =
        valid && Boolean(mediaTypes[VIDEO] && mediaTypes[VIDEO].playerSize);
    } else if (mediaTypes && mediaTypes[NATIVE]) {
    ...
    for (let i = 0; i < serverResponse.body.length; i++) {
      ...
      if (isBidResponseValid(resItem)) {
        const advertiserDomains =
          resItem.adomain && resItem.adomain.length ? resItem.adomain : [];
  ...
  getUserSyncs: (
    ...
    if (gdprConsent && gdprConsent.consentString) {
      if (typeof gdprConsent.gdprApplies === 'boolean') {
        syncUrl += `&gdpr=${Number(gdprConsent.gdprApplies)}&gdpr_consent=${
