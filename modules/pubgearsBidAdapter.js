...
const buildEndpointUrl = (serverId) => `https://${serverId}.pubgears.com/openrtb2/auction`
...
export const spec = {
  ...
  isBidRequestValid: function (bid) {
    if (bid && bid?.params) {
      return Boolean(bid.params?.publisherId) && Boolean(bid.params?.tagId) && Boolean(bid.params?.serverId);
    }
    return false;
  },
  buildRequests: function (validBidRequests, bidderRequest) {
    const request = {
      method: 'POST',
      url: buildEndpointUrl(validBidRequests[0].params.serverId),
      data: null,
      options: {},
      bidderRequest
    };
    ...
    validBidRequests.forEach((bidRequest) => {
      let newReq = newOrtbBidRequest(bidRequest, bidderRequest, currentImps);
      currentImps = newReq.imp;

      request.data = JSON.stringify(newReq);
    })
    ...
    try {
      if (response.body && response.body.seatbid && isArray(response.body.seatbid)) {
        ...
        response.body.seatbid.forEach(seatbidder => {
          seatbidder.bid && isArray(seatbidder.bid) &&
  ...
  getUserSyncs: function (syncOptions, responses, gdprConsent, uspConsent) {
    ...
    if (!hasSynced && (syncOptions.iframeEnabled || syncOptions.pixelEnabled)) {
        ...
        if (csResp.body && csResp.body.ext && csResp.body.ext.usersyncs) {
          try {
            ...
            bidders.forEach(bidder => {
              if (bidder.no_cookie) {
                let url = bidder.usersync.url;
                let type = bidder.usersync.type;

                if (!url) {
                  logError(`No sync url for pubgears bidder.`);
                } else if ((type === 'image' || type === 'redirect') && syncOptions.pixelEnabled) {
                  logMessage(`Invoking image pixel user sync for pubgears`);
                  allUserSyncs.push({type: 'image', url: url});
                } else if (type == 'iframe' && syncOptions.iframeEnabled) {
                  logMessage(`Invoking iframe user sync for pubgears`);
                  allUserSyncs.push({type: 'iframe', url: url});
                } else {
                  logError(`User sync type "${type}" not supported for pubgears`);
                }
              }
            })
          } catch (e) {
      ...
      logWarn('Pubgears: Please enable iframe/pixel based user sync.');
    }
...
export function hasValidSupplyChainParams(schain) {
  ...
  if (!isValid) logError('Pubgears: required schain params missing');
  return isValid;
}
...
function newOrtbBidRequest(bidRequest, bidderRequest, currentImps) {
  ...
  if (isFn(bidRequest.getFloor) && !config.getConfig('disableFloors')) {
    ...
    try {
      ...
      logError('Pubgears: getFloor threw an error: ', e);
    }
