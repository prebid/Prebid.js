...
const BIDDER_CODE = 'afront';
const ACCOUNTID_MACROS = '[account_id]';
const URL_ENDPOINT = `https://snt1.afront.io/?rsd=prebidjs&sk=${ACCOUNTID_MACROS}`;
...
export const spec = {
  ...
  buildRequests: (validBidRequests, bidderRequest) => {
    ...
    for (let bidRequest of validBidRequests) {
      ...
      let data = {
        ...
        site: {
          page: winLocation.pathname,
          host: winLocation.host
        },
        source: {
  ...
  interpretResponse: (serverResponse) => {
    ...
    let AfrontResponse = serverResponse.body;

    let bids = [];
    for (let response of AfrontResponse) {
...
const addNativeParameters = bidRequest => {
  ...
  const assets = _map(bidRequest.mediaTypes.native, (bidParams, key) => {
    ...
    if (props) {
      ...
      if (bidParams.sizes) {
        const sizes = [].concat(...bidParams.sizes);
        wmin = sizes[0];
        hmin = sizes[1];
      }
