...
function getPlacementInfo(bidReq) {
  ...
  try {
    const {distanceToView, elementHeight} = getViewableDistance(placementElementNode)
    return cleanObj({
...
function getViewableDistance(element) {
  ...
  return {distanceToView: Math.round(elementRect.top - targetTop), elementHeight: elementRect.height}
}
