import { precisoIdSubmodule, storage } from 'modules/precisoIdSystem.js';
...
describe('precisoId submodule', () => {
  it('should expose a "name" property containing precisoId', () => {
    expect(precisoIdSubmodule.name).to.equal('precisoId');
  });

  it('should expose a "gvlid" property containing the GVL ID 874', () => {
    expect(precisoIdSubmodule.gvlid).to.equal(874);
  });
});

describe('decode', () => {
  it('should respond with an object with "precisoId" key containing the value', () => {
    expect(precisoIdSubmodule.decode(TEST_ID)).to.deep.equal({
      precisoId: TEST_ID
    });
  });
  ...
  it('should respond with undefined if the value is not a string', () => {
    [1, null, undefined, NaN, [], {}].forEach((value) => {
      expect(precisoIdSubmodule.decode(value)).to.equal(undefined);
    });
  });
});
...
describe('getId', () => {
  ...
  it('should call the sync endpoint and accept a valid response', () => {
    storage.setDataInLocalStorage('_pre|usrid15', TEST_ID);
    ...
    expect(search.av).to.equal(precisoIdSubmodule.version);
