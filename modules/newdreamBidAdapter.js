...
export const spec = {
  ...
  buildRequests: function (bidRequests, bidderRequest) {
    ...
    if (debugCookie) {
      try {
        ...
        logError('Newdream Debug Auction Cookie Error:\n\n' + e);
      }
    } else {
    ...
    if (debugObjParams.enabled) {
      ...
      logInfo('Newdream Debug Auction Settings:\n\n' + JSON.stringify(debugObjParams, null, 4));
    }
  ...
  interpretResponse: function (serverResponse, { bidderRequest }) {
    ...
    if (serverResponse.debug && serverResponse.debug.debug_info) {
      const debugHeader = 'Newdream Debug Auction for Prebid\n\n'
...
function newRenderer(adUnitCode, rtbBid, rendererOptions = {}) {
  ...
  renderer.setEventHandlers({
    impression: () => logMessage('Newdream outstream video impression event'),
    loaded: () => logMessage('Newdream outstream video loaded event'),
    ended: () => {
      logMessage('Newdream outstream renderer video event');
