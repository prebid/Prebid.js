...
The module also forwards the publisher's consent strings to the cloud as evidence when present. The TCF consent string (from Prebid's GDPR consent) is sent as `tcstring` and the GPP string (from Prebid's GPP consent) is sent as `gppstring`; the cloud can infer the marketing usage preference from either when no explicit `id.usage` is provided and PMP is not present, so 51DiD works for publishers running any TCF or GPP CMP. An explicit `id.usage` always takes precedence. These come from Prebid's consent data, not module params, and are sent alongside `id.usage`.
...
| params.idUsage        | String  | Marketing usage preference forwarded to the cloud as the `id.usage` evidence — see [51DiD documentation](https://51degrees.com/documentation/4.5/_identifiers_51_did.html) for accepted values (`non-marketing`, `standard`, `personalized`). When omitted, the module reads `localStorage['__51d_pmp_pref']` written by the [51Degrees Preference Management Platform](https://51degrees.com/documentation/4.5/_identifiers__p_m_p.html); when neither is set the cloud silently skips 51DiD production. |                    |
