...
import {
  convertTypes
} from '../libraries/transformParamsUtils/convertTypes.js';
...
function digForAppNexusBidder(s2sConfig) {
  ...
  // check for server-side alias
  if (!result && isPlainObject(s2sConfig?.extPrebid?.aliases) && (Object.values(s2sConfig?.extPrebid?.aliases).includes('appnexus'))) result = true;
...
export function convertAnParams(next, bidderRequests) {
  ...
  // check for dynamic aliases (from pbjs.aliasBidder)
  ...
  if (proceed) {
    bidderRequests.forEach(bidRequest => {
      if (isThisOurBidderInDisguise(bidRequest.bidderCode, s2sConfig)) {
