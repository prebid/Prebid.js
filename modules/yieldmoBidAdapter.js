...
const OUTSTREAM_VIDEO_PLAYER_URL =
  "https://prebid-outstream.yieldmo.com/bundle.js";
...
const BANNER_REQUEST_PROPERTIES_TO_REDUCE = [
  "description",
  "title",
  "pr",
  "page_url",
...
export const spec = {
  ...
  isBidRequestValid: function (bid) {
    return !!(
      ...
      bid.adUnitCode &&
      bid.bidId &&
      (hasBannerMediaType(bid) || hasVideoMediaType(bid)) &&
  ...
  buildRequests: function (bidRequests, bidderRequest) {
    ...
    const bannerBidRequests = bidRequests.filter((request) =>
      hasBannerMediaType(request)
    );
    const videoBidRequests = bidRequests.filter((request) =>
      hasVideoMediaType(request)
    );
    let serverRequests = [];
    ...
    if (bannerBidRequests.length > 0) {
      ...
      if (canAccessTopWindow()) {
        serverRequest.pr =
          (LOCAL_WINDOW.document && LOCAL_WINDOW.document.referrer) || "";
      ...
      const fullUrl = `${bannerUrl}?${parseQueryStringParameters(
        serverRequest
      )}`;
      ...
      if (extraCharacters > 0) {
        for (let i = 0; i < BANNER_REQUEST_PROPERTIES_TO_REDUCE.length; i++) {
          extraCharacters = shortcutProperty(
            extraCharacters,
            serverRequest,
            BANNER_REQUEST_PROPERTIES_TO_REDUCE[i]
          );
  ...
  interpretResponse: function (serverResponse, bidRequest) {
    ...
    if (data.seatbid) {
      const seatbids = data.seatbid.reduce(
  ...
  getUserSyncs: function (
    ...
    const gdprString = `&gdpr_consent=${encodeURIComponent(
...
function createNewVideoBid(response, bidRequest) {
  const imp = find(
    deepAccess(bidRequest, "data.imp") || [],
    (imp) => imp.id === response.impid
  );
...
function getPageDescription() {
  if (document.querySelector('meta[name="description"]')) {
    ...
    ); // Value of the description metadata from the publisher's page.
  } else {
    return "";
  }
}
...
function getId(request, idType) {
  return typeof deepAccess(request, "userId") === "object"
...
function validateVideoParams(bid) {
  ...
  const validate = (fieldPath, validateCb, errorCb, errorCbParam) => {
    if (fieldPath.indexOf("video") === 0) {
      ...
      if (hasValidValueParams) return valueParams;
        ...
        if (!hasValidValueParams)
          errorCb(valueFieldPath, valueParams, errorCbParam);
        else if (!hasValidMediaTypesParams)
  ...
  try {
    ...
    validate(
      ...
      paramInvalid,
      "array of 2 integers, ex: [640,480] or [[640,480]]"
    );

    validate("video.mimes", (val) => isDefined(val), paramRequired);
    validate(
      "video.mimes",
      (val) => isArray(val) && val.every((v) => isStr(v)),
      paramInvalid,
    ...
    validate(
      "video.protocols",
      (val) => isArrayOfNums(val) && val.every((v) => v >= 1 && v <= 6),
      paramInvalid,
      "array of numbers, ex: [2,3]"
    );
    ...
    validate(
      ...
      (val) => !isDefined(val) || isArrayOfNums(val),
      paramInvalid,
    ...
    validate(
      "video.minduration",
      (val) => !isDefined(val) || isNumber(val),
      paramInvalid
    );
    validate(
      "video.skippable",
      (val) => !isDefined(val) || isBoolean(val),
      paramInvalid
    );
    validate(
      "video.skipafter",
...
function shortcutProperty(extraCharacters, target, propertyName) {
  if (target[propertyName].length > extraCharacters) {
    target[propertyName] = target[propertyName].substring(
      0,
      target[propertyName].length - extraCharacters
    );
