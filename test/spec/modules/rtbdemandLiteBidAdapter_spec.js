...
import {spec} from 'modules/rtbdemandLiteBidAdapter';
import {getTopWindowLocation} from 'src/utils';
import {newBidder} from 'src/adapters/bidderFactory';

describe('Rtbdemand Lite Adapter Tests', () => {
  const slotConfigs = [{
  ...
  it('Verify build request', () => {
    const request = spec.buildRequests(slotConfigs)[0];
    expect(request.url).to.equal('//bidding.rtbdemand.com/hb1.php?hb=2&aff=p10000');
  ...
  it('Verify Native request', () => {
    const request = spec.buildRequests(nativeSlotConfig)[0];
    expect(request.url).to.equal('//bidding.rtbdemand.com/hb1.php?hb=2&aff=p10000');
    expect(request.method).to.equal('POST');
    const ortbRequest = JSON.parse(request.data);
    // native impression
    ...
    // data asset
    expect(nativeRequest.assets[1].id).to.equal(3);
    ...
    // image asset
    expect(nativeRequest.assets[2].id).to.equal(5);
  ...
  it('Verify Native response', () => {
    ...
    const nativeResponse = {
      'native': {
        assets: [
          { title: { text: 'Ad Title'} },
          { data: { id: 3, value: 'Sponsored By: Brand' }},
          { img: { id: 5, url: 'http://images.cdn.brand.com/123' } }
        ],
  ...
  it('Verifies bidder code', () => {
    expect(spec.code).to.equal('rtbdemandLite');
  });

  it('Verifies bidder aliases', () => {
    expect(spec.aliases).to.have.lengthOf(1);
    expect(spec.aliases[0]).to.equal('rtbdemandLite');
  });
  ...
  it('Verify app requests', () => {
    const request = spec.buildRequests(appSlotConfig)[0];
    const ortbRequest = JSON.parse(request.data);
    // site object
    ...
    expect(ortbRequest.app.bundle).to.equal('com.rtbdemand.apps');
    expect(ortbRequest.app.storeurl).to.equal('http://rtbdemand.com/apps');
    expect(ortbRequest.app.domain).to.equal('rtbdemand.com');
  });
});
