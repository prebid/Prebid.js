...
=======
>>>>>>> cd32d64... added mraid check and catch for window.top
    });
  ...
  describe('buildRequests', () => {
    ...
    it('should place bid information into the p parameter of data', () => {
      ...
      expect(addPlacement(bid)).to.be.equal('{"placement_id":"adunit-code","callback_id":"30b31c1838de1e","sizes":[[300,250],[300,600]]}');

      const placementInfo = spec.buildRequests([bid]).data.p;
      expect(placementInfo[0]).to.be.equal('{"placement_id":"adunit-code","callback_id":"30b31c1838de1e","sizes":[[300,250],[300,600]]}');
    });

    it('should add additional information to data parameter of request', () => {
      const data = spec.buildRequests([bid]).data;
      expect(data.hasOwnProperty('page_url')).to.be.true;
      expect(data.hasOwnProperty('bust')).to.be.true;
      expect(data.hasOwnProperty('pr')).to.be.true;
      expect(data.hasOwnProperty('scrd')).to.be.true;
      expect(data.hasOwnProperty('dnt')).to.be.true;
    ...
    it('should not add responses if the cpm is 0 or null', () => {
      serverResponse[0].cpm = 0;
      let response = spec.interpretResponse(serverResponse);
      expect(response).to.deep.equal([]);

      serverResponse[0].cpm = null;
      response = spec.interpretResponse(serverResponse);
      expect(response).to.deep.equal([])
    });
  });
  ...
  describe('getUserSync', () => {
    ...
    it('should not return a tracker when not in ios', () => {
      // not ios, so tracker will fail
      expect(spec.getUserSync(options)).to.deep.equal([]);
    });
  });
  ...
  describe('getUserSync', () => {
    ...
    it('should return a tracker with type and url as parameters', () => {
      // iframeEnabled and pixel enabled
      expect(spec.getUserSync(options)).to.deep.equal([{
        type: 'iframe',
        url: SYNC_ENDPOINT
      }]);
    ...
    //   expect(spec.getUserSync(options)).to.equal(undefined);
    });
  });
});
