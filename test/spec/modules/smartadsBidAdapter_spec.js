...
import { spec, ENDPOINT, BIDDER_CODE } from '../../../modules/smartadsBidAdapter.js'
...
describe('smartadsBidAdapter', () => {
    ...
    describe('buildRequests', () => {
        ...
        config.setBidderConfig({
            bidders: ['smartads'],
            config: smartadsBidderConfig
        })



        const bidList = [bidItem]
        const request = config.runWithBidder(BIDDER_CODE, () => spec.buildRequests(bidList, bidRequest))
        ...
        it('should be create a request to server with POST method, data, valid url', () => {
            ...
            expect(request.url).equal(ENDPOINT + smartadsBidderConfig.site.fosp_uid)
        })
        it('should return valid data format if bid array is valid', () => {
            expect(_data).to.be.an('object')
            expect(_data).to.has.all.keys('deviceWidth', 'deviceHeight', 'language', 'host', 'page', 'imp', 'device', 'myvne_id', 'orig_aid', 'fosp_aid', 'fosp_uid', 'id')
            ...
            expect(_data.page).to.be.an("string").that.is.equal("https://page.example.com/here.html")
        ...
        it("should return empty array if there is no bidItem passed", () => {
            const request = config.runWithBidder(BIDDER_CODE, () => spec.buildRequests([], bidRequest))
            const _data = request.data
        ...
        it("have to contain required params and correct format for sending to smartAds", () => {
    ...
    describe('interpretResponse', () => {
        ...
        it('should return correct format, params for each of offers from server response', () => {
            ...
            expect(offer.adserverTargeting["hb_ad_smartads"]).to.be.an("string").that.is.not.empty
        })
    })
})
