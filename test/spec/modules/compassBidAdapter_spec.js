...
describe('CompassBidAdapter', function () {
  ...
  describe('getUserSyncs', function() {
    it('Should return array of objects with proper sync config , include GDPR', function() {
      const syncData = config.runWithBidder(bidder, () => spec.getUserSyncs({ pixelEnabled: true }, {}, {
        consentString: 'ALL',
        gdprApplies: true,
      }, undefined));
    ...
    it('Should return array of objects with proper sync config , include CCPA', function() {
      const syncData = config.runWithBidder(bidder, () => spec.getUserSyncs({ pixelEnabled: true }, {}, {}, '1---'));
      expect(syncData).to.be.an('array').which.is.not.empty;
      expect(syncData[0]).to.be.an('object')
      expect(syncData[0].type).to.be.a('string')
      expect(syncData[0].type).to.equal('image')
    ...
    it('Should return array of objects with proper sync config , include GPP', function() {
      const syncData = config.runWithBidder(bidder, () => spec.getUserSyncs({ pixelEnabled: true }, {}, {}, undefined, {
        gppString: 'abc123',
        applicableSections: [8]
      }));
      expect(syncData).to.be.an('array').which.is.not.empty;
      ...
      expect(syncData[0].type).to.equal('image')
      expect(syncData[0].url).to.be.a('string')
      expect(syncData[0].url).to.equal('https://sa-cs.deliverimp.com/image?pbjs=1&gpp=abc123&gpp_sid=8&coppa=0')
    });
  });
});
