...
describe('smaatoBidAdapterTest', () => {
  ...
  describe('buildRequests', () => {
    ...
    describe('common', () => {
      ...
      it('sends floor when configured', () => {
        ...
        const req = extractPayloadOfFirstAndOnlyRequest(reqs);
        expect(req.imp[0].floor).to.be.equal(0.123);
      });

      it('sends default floor when invalid', () => {
        const singleBannerBidRequestWithFloor = Object.assign({}, singleBannerBidRequest);
        singleBannerBidRequestWithFloor.getFloor = function() {
          return undefined;
        }
        ...
        const req = extractPayloadOfFirstAndOnlyRequest(reqs);
        expect(req.imp[0].floor).to.be.equal(0);
      });

      it('sends default floor when not a number', () => {
        const singleBannerBidRequestWithFloor = Object.assign({}, singleBannerBidRequest);
        singleBannerBidRequestWithFloor.getFloor = function() {
          return {
            currency: 'USD',
      ...
      it('sends default floor when wrong currency', () => {
        const singleBannerBidRequestWithFloor = Object.assign({}, singleBannerBidRequest);
        singleBannerBidRequestWithFloor.getFloor = function() {
          return {
            currency: 'EUR',
    ...
    describe('buildRequests for video imps', () => {
      ...
      it('sends floor when configured', () => {
        ...
        const req = extractPayloadOfFirstAndOnlyRequest(reqs);
        expect(req.imp[0].floor).to.be.equal(0.456);
      });
      ...
      describe('ad pod / long form video', () => {
        describe('required parameters with requireExactDuration false', () => {
          ...
          it('sends required fields', () => {
            ...
            expect(req.imp[1].floor).to.be.equal(0);
          ...
          it('sends floor when configured', () => {
            ...
            const req = extractPayloadOfFirstAndOnlyRequest(reqs);
            expect(req.imp[0].floor).to.be.equal(0.789);
            expect(req.imp[1].floor).to.be.equal(0.789);
          });
