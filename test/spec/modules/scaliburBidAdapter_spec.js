...
import { spec, getFirstPartyUUID,
  getAndSetFirstPartyData,
  storage
} from 'modules/scaliburBidAdapter.js';
...
describe('Scalibur Adapter', function () {
  ...
  describe('getFirstPartyUUID', function () {
    it('should generate a valid UUID format', function () {
      const uuid = getFirstPartyUUID();
    ...
    it('should generate unique UUIDs', function () {
      const uuid1 = getFirstPartyUUID();
      const uuid2 = getFirstPartyUUID();

      expect(uuid1).to.not.equal(uuid2);
    });

    it('should generate 32 character UUID without hyphens', function () {
      const uuid = getFirstPartyUUID();
      const withoutHyphens = uuid.replace(/-/g, '');

      expect(withoutHyphens).to.have.lengthOf(32);
      expect(withoutHyphens).to.match(/^[0-9a-f]{32}$/);
    });

    it('should handle Math.random edge cases', function () {
      const originalRandom = Math.random;
      let callCount = 0;

      // Mock Math.random to return predictable values
      Math.random = () => {
        // Return 0 for even calls, 0.99999 for odd calls
        return callCount++ % 2 === 0 ? 0 : 0.999999;
      };
    ...
    it('should generate UUID with correct hyphen positions', function () {
      const uuid = getFirstPartyUUID();
  ...
  describe('getAndSetFirstPartyData', function () {
    let sandbox;
    let storageStub;
    ...
    beforeEach(function () {
      ...
      sandbox.stub(storage, 'setDataInLocalStorage').callsFake(storageStub.setDataInLocalStorage);
    });
    ...
    it('should generate and store new first party data when none exists', function () {
      storageStub.hasLocalStorage.returns(true);
      storageStub.getDataFromLocalStorage.returns(null);

      const result = getAndSetFirstPartyData();

      // Should have generated new data
      expect(result.pcid).to.be.a('string');
      expect(result.pcidDate).to.be.a('number');

      // UUID should be valid format
      const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/;
      expect(result.pcid).to.match(uuidRegex);

      // Should have stored the data
      expect(storageStub.setDataInLocalStorage.calledOnce).to.be.true;
      expect(storageStub.setDataInLocalStorage.calledWith('_iiq_fdata')).to.be.true;

      const storedData = JSON.parse(storageStub.setDataInLocalStorage.getCall(0).args[1]);
      expect(storedData.pcid).to.equal(result.pcid);
      expect(storedData.pcidDate).to.equal(result.pcidDate);
    });
    ...
    it('should handle empty string localStorage data', function () {
      storageStub.hasLocalStorage.returns(true);
      storageStub.getDataFromLocalStorage.returns('');

      const result = getAndSetFirstPartyData();

      // Should generate new data when storage returns empty string
      expect(result.pcid).to.be.a('string');
      expect(result.pcidDate).to.be.a('number');
      expect(storageStub.setDataInLocalStorage.calledOnce).to.be.true;
    });

    it('should handle JSON.parse throwing an error', function () {
      storageStub.hasLocalStorage.returns(true);
      storageStub.getDataFromLocalStorage.returns('{"invalid": json}');

      const result = getAndSetFirstPartyData();

      // Should generate new data when JSON parsing fails
      expect(result.pcid).to.be.a('string');
      expect(result.pcidDate).to.be.a('number');
      expect(storageStub.setDataInLocalStorage.calledOnce).to.be.true;
    });

    it('should handle partial data in localStorage', function () {
      const partialData = { pcid: 'test-uuid' }; // missing pcidDate

      storageStub.hasLocalStorage.returns(true);
      storageStub.getDataFromLocalStorage.returns(JSON.stringify(partialData));

      const result = getAndSetFirstPartyData();

      // Should use the existing data even if partial
      expect(result.pcid).to.equal(partialData.pcid);
      expect(result.pcidDate).to.be.undefined;
      expect(storageStub.setDataInLocalStorage.called).to.be.false;
    });
  });
