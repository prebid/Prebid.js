    ...
    bids: [
      {
        ...
        params: {
          ...
          publisher: '52',
          position: 'left',
          referrer: 'https://trace.mediagotechnology.com',
          bidfloor: 0.01,
          ortb2Imp: {
            ext: {
              gpid: 'adslot_gpid',
...
describe('mediagoTechnology: onBidWon', function() {
  ...
  it('should call triggerPixel when nurl exists', function() {
    spec.onBidWon({ nurl: 'https://trace.mediagotechnology.com/win?id=123' });
    expect(utils.triggerPixel.calledOnce).to.be.true;
    expect(utils.triggerPixel.calledWith('https://trace.mediagotechnology.com/win?id=123')).to.be.true;
  });
...
describe('mediagoTechnology: Native Ad Support', function() {
  ...
  describe('interpretResponse with native bids', function() {
    const nativeAdm = JSON.stringify({
      assets: [
        { id: 1, img: { type: 3, url: 'https://images.mediagotechnology.com/img/300x157.png', w: 300, h: 157 } },
        { id: 2, title: { text: 'Test Native Ad Title', len: 20 } },
        { id: 3, data: { type: 1, value: 'TestSponsor' } }
      ],
      link: { url: 'https://trace.mediagotechnology.com/ju/ic?tn=test&ap={AUCTION_PRICE}' },
      eventtrackers: [
        { event: 1, method: 1, url: 'https://trace.mediagotechnology.com/ju/imp?tn=test' },
        { event: 2, method: 1, url: 'https://trace.mediagotechnology.com/ju/view?tn=test' }
      ],
      privacy: 'https://cdn.mediagotechnology.com/js/officialWebsite/privacy.html'
    });
      ...
      body: {
        ...
        seatbid: [{
          ...
          bid: [{
            ...
            nurl: 'https://trace.mediagotechnology.com/ju/win?tn=test&ap=${AUCTION_PRICE}'
          }]
        }],
        cur: 'USD'
      }
    ...
    it('should return correct native bid response with all fields', function() {
      ...
      expect(bid.nurl).to.equal('https://trace.mediagotechnology.com/ju/win?tn=test&ap=${AUCTION_PRICE}');
    ...
    it('should parse native ortb object correctly', function() {
      ...
      expect(ortb.assets[0].img.url).to.equal('https://images.mediagotechnology.com/img/300x157.png');
      expect(ortb.assets[1].title.text).to.equal('Test Native Ad Title');
      expect(ortb.assets[2].data.value).to.equal('TestSponsor');
      expect(ortb.link.url).to.include('trace.mediagotechnology.com');
      expect(ortb.eventtrackers).to.have.lengthOf(2);
      expect(ortb.eventtrackers[0].event).to.equal(1);
      expect(ortb.privacy).to.equal('https://cdn.mediagotechnology.com/js/officialWebsite/privacy.html');
    });

    it('should handle adm wrapped with native key', function() {
      const wrappedAdm = JSON.stringify({
        native: {
          assets: [{ id: 1, img: { type: 3, url: 'https://images.mediagotechnology.com/img/wrapped.png', w: 300, h: 157 } }],
          link: { url: 'https://trace.mediagotechnology.com/ju/ic?wrapped=1' }
        }
      });
      ...
      expect(bids).to.have.lengthOf(1);
      expect(bids[0].native.ortb.assets[0].img.url).to.equal('https://images.mediagotechnology.com/img/wrapped.png');
