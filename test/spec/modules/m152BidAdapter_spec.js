...
describe('M152BidAdapter', function () {
  const userIdAsEids = [{
    source: 'test.org',
    uids: [{
      id: '01**********',
  ...
  describe('buildRequests', function () {
    ...
    it('Returns valid EU URL', function () {
      ...
      expect(serverRequest.url).to.equal('https://eu.152media-ssp.com/pbjs');
    });

    it('Returns valid US-EAST URL', function () {
      bids[0].params.region = 'us-east';
      const serverRequest = spec.buildRequests(bids, bidderRequest);
      expect(serverRequest.url).to.equal('https://east.152media-ssp.com/pbjs');
    });
  ...
  describe('getUserSyncs', function() {
    ...
    it('Should return array of objects with proper sync config , include GDPR', function() {
      ...
      expect(syncData[0].url).to.equal('https://cs.152media-ssp.com/image?pbjs=1&gdpr=1&gdpr_consent=ALL&coppa=0');
    });
    it('Should return array of objects with proper sync config , include CCPA', function() {
      ...
      expect(syncData[0].type).to.equal('image');
      expect(syncData[0].url).to.be.a('string');
      expect(syncData[0].url).to.equal('https://cs.152media-ssp.com/image?pbjs=1&ccpa_consent=1---&coppa=0');
    });
    it('Should return array of objects with proper sync config , include GPP', function() {
      ...
      expect(syncData[0].type).to.equal('image');
      expect(syncData[0].url).to.be.a('string');
      expect(syncData[0].url).to.equal('https://cs.152media-ssp.com/image?pbjs=1&gpp=abc123&gpp_sid=8&coppa=0');
    });
  });
});
