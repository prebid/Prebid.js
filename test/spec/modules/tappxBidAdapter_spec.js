...
describe('Tappx bid adapter', function () {
  ...
  describe('buildRequest', function () {
    ...
    it('should correctly build ortb2Imp.ext params when present', function () {
      const bidRequestsWithAdData = c_VALIDBIDREQUESTS;
      const request = spec.buildRequests(bidRequestsWithAdData, bidderRequest);
      const payload = JSON.parse(request[0].data);
      const impExt = payload.imp[0].ext;

      expect(impExt.data).to.exist.and.to.be.an('object');
      expect(impExt.data.pbadslot).to.be.equal('/19968336/header-bid-tag-0');

      expect(impExt.data.adserver).to.exist.and.to.be.an('object');
      expect(impExt.data.adserver.name).to.be.equal('gam');
      expect(impExt.data.adserver.adslot).to.be.equal('/19968336/header-bid-tag-0');

      expect(impExt.divid).to.be.equal('div-ad-12345');
      expect(impExt.gpid).to.be.equal('/19968336/header-bid-tag-0');
    });

    it('should not add ortb2Imp.ext params when not received', function () {
      const bidRequestsWithoutGpidData = c_VALIDBIDREQUESTS;

      bidRequestsWithoutGpidData[0].ortb2Imp.ext = {
        gpid: undefined,
        data: {
          pbadslot: undefined,
          adserver: {
      ...
      const request = spec.buildRequests(bidRequestsWithoutGpidData, bidderRequest);
      const payload = JSON.parse(request[0].data);
      const impExt = payload.imp[0].ext;

      expect(impExt.data).to.not.exist;
      expect(impExt.divid).to.be.an('undefined');
      expect(impExt.gpid).to.be.an('undefined');
    });
  });
  ...
  describe('interpretResponse', function () {
    it('receive banner reponse with single plcmt', function () {
    ...
    it('receive video reponse with single plcmt', function () {
  ...
  describe('_getHostInfo tests', function() {
    ...
    it('Test testing endpoints', function() {
      const testHostValidRequest = HOST_VALIDBIDREQ;
      testHostValidRequest.params.host = 'testing.xxx.tappx.com\/rtb\/v2\/';
      const testHostObject = _getHostInfo(testHostValidRequest);
    ...
    it('Test classic endpoints', function() {
      const classicHostValidRequest = HOST_VALIDBIDREQ;
      classicHostValidRequest.params.host = 'xxx.xxx.tappx.com\/rtb\/v2\/';
      const classicHostObject = _getHostInfo(classicHostValidRequest);
    ...
    it('Test new endpoints', function() {
      const newHostValidRequest = HOST_VALIDBIDREQ;
      newHostValidRequest.params.host = 'zz1111xxx.xxx.tappx.com\/rtb\/v2\/';
      const newHostObject = _getHostInfo(newHostValidRequest);
  ...
  describe('Empty params values from bid tests', function() {
    const validBidRequest = JSON.parse(JSON.stringify(c_BIDREQUEST));
