...
describe('SmartHubBidAdapter', function () {
  ...
  describe('getUserSyncs', function() {
    it('Should return array of objects with GDPR values', function() {
      ...
      expect(syncData[0].url).to.equal('https://us4.shb-sync.com/image?pid=360&gdpr=1&gdpr_consent=ALL&coppa=0')
    });
    it('Should return array of objects with CCPA values', function() {
      ...
      expect(syncData[0].type).to.equal('image')
      expect(syncData[0].url).to.be.a('string')
      expect(syncData[0].url).to.equal('https://us4.shb-sync.com/image?pid=360&ccpa_consent=1---&coppa=0')
    });
    it('Should return array of objects with GPP values', function() {
      ...
      expect(syncData[0].type).to.equal('image')
      expect(syncData[0].url).to.be.a('string')
      expect(syncData[0].url).to.equal('https://us4.shb-sync.com/image?pid=360&gpp=ab12345&gpp_sid=8&coppa=0')
    });
    it('Should return iframe type if iframeEnabled is true', function() {
      ...
      expect(syncData[0].url).to.be.a('string')
      expect(syncData[0].url).to.equal('https://us4.shb-sync.com/iframe?pid=360&coppa=0')
    });
  });
});
