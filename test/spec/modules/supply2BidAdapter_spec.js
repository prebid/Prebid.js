...
import { spec } from 'modules/supply2BidAdapter';
import { newBidder } from 'src/adapters/bidderFactory';

describe('Supply2Adapter', function () {
  ...
  describe('buildRequests', function () {
    ...
    it('should attach valid params to the tag', function () {
      ...
      expect(payload).to.have.property('auids', '16');
    ...
    it('auids must not be duplicated', function () {
      ...
      expect(payload).to.have.property('u').that.is.a('string');
      expect(payload).to.have.property('pt', 'net');
      expect(payload).to.have.property('auids', '16,17');
      expect(payload).to.have.property('r', '22edbae2733bf6');
    });

    it('if timeout is present, payload must have wtimeout parameter', function () {
      const request = spec.buildRequests(bidRequests, {timeout: 2000});
      expect(request.data).to.be.an('string');
      const payload = parseRequest(request.data);
      expect(payload).to.have.property('wtimeout', '2000');
    });
  ...
  describe('interpretResponse', function () {
    const responses = [
      {'bid': [{'price': 1.15, 'adm': '<div>test content 1</div>', 'auid': 23, 'h': 250, 'w': 300}], 'seat': '1'},
      {'bid': [{'price': 0.5, 'adm': '<div>test content 2</div>', 'auid': 24, 'h': 90, 'w': 728}], 'seat': '1'},
      {'bid': [{'price': 0, 'auid': 25, 'h': 250, 'w': 300}], 'seat': '1'},
