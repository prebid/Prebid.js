...
describe('GambidAdapter', () => {
  ...
  describe('interpretResponse', () => {
    ...
    it('aggregates user-sync pixels', () => {
      ...
      expect(response[ 0 ].type).to.equal(rtbResponse.ext.pixels[ 0 ].type);
      expect(response[ 0 ].url).to.equal(rtbResponse.ext.pixels[ 0 ].url);
      expect(response[ 1 ].type).to.equal(rtbResponse.ext.pixels[ 1 ].type);
      expect(response[ 1 ].url).to.equal(rtbResponse.ext.pixels[ 1 ].url);
      expect(response[ 2 ].type).to.equal(rtbResponse.seatbid[ 0 ].bid[ 0 ].ext.pixels[ 0 ].type);
      expect(response[ 2 ].url).to.equal(rtbResponse.seatbid[ 0 ].bid[ 0 ].ext.pixels[ 0 ].url);
      expect(response[ 3 ].type).to.equal(rtbResponse.seatbid[ 1 ].bid[ 0 ].ext.pixels[ 0 ].type);
      expect(response[ 3 ].url).to.equal(rtbResponse.seatbid[ 1 ].bid[ 0 ].ext.pixels[ 0 ].url);
    });
  });
});
