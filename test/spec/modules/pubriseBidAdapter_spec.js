...
import { spec } from '../../../modules/pubriseBidAdapter.js';
import { BANNER, VIDEO, NATIVE } from '../../../src/mediaTypes.js';
import { getUniqueIdentifierStr } from '../../../src/utils.js';

const bidder = 'pubrise';

describe('PubriseBidAdapter', function () {
  const userIdAsEids = [{
    source: 'test.org',
    uids: [{
      id: '01**********',
  ...
  describe('buildRequests', function () {
    ...
    it('Returns valid URL', function () {
      expect(serverRequest.url).to.equal('https://backend.pubrise.ai/pbjs');
    });
  ...
  describe('getUserSyncs', function() {
    ...
    it('Should return array of objects with proper sync config , include GPP', function() {
      ...
      expect(syncData[0].type).to.equal('image')
      expect(syncData[0].url).to.be.a('string')
      expect(syncData[0].url).to.equal('https://sync.pubrise.ai/image?pbjs=1&gpp=abc123&gpp_sid=8&coppa=0')
    });
  });
});
