describe('mobfox adapter tests', function () {
  ...
  describe('test valid MF request failed1', function () {
    let isValid = adapter.spec.isBidRequestValid(bidRequestInvalid1[0]);
    expect(isValid).to.equal(false);
  });

  describe('test valid MF request failed2', function () {
    let isValid = adapter.spec.isBidRequestValid(bidRequestInvalid2[0]);
    expect(isValid).to.equal(false);
  });

  describe('test build MF request', function () {
    const request = adapter.spec.buildRequests(bidRequest);
    ...
    expect(payload[3]).to.equal('requestId=c241c810-18d9-4aa4-a62f-8c1980d8d36b');
    expect(payload[4]).to.equal('s=267d72ac3f77a3f447b32cf7ebf20673');
    expect(payload[6]).to.equal('adspace_width=320');
    expect(payload[7]).to.equal('adspace_height=480');
    expect(payload[8]).to.equal('imp_instl=1');
  });
  ...
  describe('test intercept response', function () {
    const request = adapter.spec.buildRequests(bidRequest);
    ...
    expect(bidResponses[0].requestId).to.equal('c241c810-18d9-4aa4-a62f-8c1980d8d36b');
    expect(bidResponses[0].ttl).to.equal(360);
    expect(bidResponses[0].width).to.equal('320');
  });

  describe('test intercept response with empty server response', function () {
    const request = adapter.spec.buildRequests(bidRequest);
    let serverResponse = {
