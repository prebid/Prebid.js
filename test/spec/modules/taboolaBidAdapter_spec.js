...
describe('Taboola Adapter', function () {
  ...
  describe('interpretResponse', function () {
    ...
    it('should return empty array if server response without seatbid', function () {
      ...
      const seatbid = {...serverResponse.body.bidResponse.seatbid[0]};
      overriddenServerResponse.body.bidResponse.seatbid[0] = {};

      const res = spec.interpretResponse(overriddenServerResponse, request)
      expect(res).to.be.an('array').that.is.empty

      overriddenServerResponse.body.bidResponse.seatbid[0] = seatbid;
    });

    it('should return empty array if server response without bids', function () {
      const overriddenServerResponse = {...serverResponse};
      const bid = [...serverResponse.body.bidResponse.seatbid[0].bid];
      overriddenServerResponse.body.bidResponse.seatbid[0].bid = {};

      const res = spec.interpretResponse(overriddenServerResponse, request)
      expect(res).to.be.an('array').that.is.empty

      overriddenServerResponse.body.bidResponse.seatbid[0].bid = bid;
    });
      ...
      const expectedRes = [
        {
          ...
          ttl: 360,
          netRevenue: false,
          currency: serverResponse.body.bidResponse.cur,
          mediaType: 'banner',
          ad: bid.adm,
          width: bid.w,
          height: bid.h,
