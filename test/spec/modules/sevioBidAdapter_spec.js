...
describe('sevioBidAdapter', function () {
  ...
  // --- Keywords detection tests (via buildRequests) ---------------------------------
  describe('keywords detection (JSON-LD / <meta> / title-H1-URL / override)', function () {
    const makeBid = (overrides = {}) => ({
      bidder: 'sevio',
      params: { zone: 'zoneId', ...(overrides.params || {}) },
      mediaTypes: overrides.mediaTypes || { banner: { sizes: [[300, 250]] } },
      bidId: overrides.bidId || 'kw-test-bid',
      bidderRequestId: 'kw-req',
      auctionId: 'kw-auction',
      refererInfo: overrides.refererInfo || {
    ...
    const makeBidderRequest = (overrides = {}) => ({
      ...
      uspConsent: { uspString: '1---' },
      gppConsent: { consentString: 'gpp' },
      ...overrides
    });

    function resetDom(path = '/some-path/article_about-cats') {
      document.head.innerHTML = '';
      document.body.innerHTML = '';
      try {
        history.replaceState({}, '', path);
      } catch (e) {
        // If replaceState is unavailable for any reason, tests still pass based on title/H1.
      }
    ...
    function setTitleAndH1(title = '', h1 = '') {
      if (title) {
    ...
    function setJsonLd(payloadOrString) {
      const s = document.createElement('script');
      s.setAttribute('type', 'application/ld+json');
      s.textContent = (typeof payloadOrString === 'string')
        ? payloadOrString
        : JSON.stringify(payloadOrString);
    ...
    it('detectJsonLdKeywords: extracts and splits JSON-LD keywords', function () {
      setJsonLd({ '@type': 'Article', keywords: 'cat,  dogs ,whiskers' });

      const reqs = spec.buildRequests([makeBid()], makeBidderRequest());
      const tokens = reqs[0].data.keywords.tokens;

      expect(tokens).to.deep.equal(['cat', 'dogs', 'whiskers']);
    });

    it('detectJsonLdKeywords: supports array of JSON-LD objects', function () {
      setJsonLd([
        { '@type': 'NewsArticle', keywords: 'alpha, beta' },
        { '@type': 'BlogPosting', keywords: 'gamma,delta' }
      ]);

      const reqs = spec.buildRequests([makeBid()], makeBidderRequest());
      const tokens = reqs[0].data.keywords.tokens;

      expect(tokens).to.deep.equal(['alpha', 'beta', 'gamma', 'delta']);
    });

    it('detectJsonLdKeywords: ignores malformed JSON and falls back', function () {
      // Malformed JSON-LD
      setJsonLd('{ "keywords": ["oops" }');
      // Fallback source
      setMetaKeywords('one, two');
    ...
    it('detectHeaderKeywords: reads <meta name="keywords"> content', function () {
      setMetaKeywords('one, two, three');
    ...
    it('detectTitleAndH1AndUrl: RAKE-like fallback yields non-empty, sensible tokens', function () {
      setTitleAndH1(
        'The Ultimate Guide to Caring for Cats & Kittens',
        'Cat Care Basics: Food, Litter, Toys'
      );
      // URL from resetDom() is: /some-path/article_about-cats

      const reqs = spec.buildRequests([makeBid()], makeBidderRequest());
      const tokens = reqs[0].data.keywords.tokens;

      expect(tokens).to.be.an('array').that.is.not.empty;
      expect(tokens.length).to.be.at.most(12);
      const lower = tokens.map(s => s.toLowerCase());
      // We just check likely terms
      expect(lower).to.include.oneOf(['cat', 'cats', 'kittens', 'care', 'guide', 'article']);
    });

    it('detectKeywords: precedence JSON-LD > meta > title/H1/URL', function () {
      // All three sources present → JSON-LD wins
      setJsonLd({ keywords: 'json,ld,win' });
      setMetaKeywords('meta,should,not,win');
      setTitleAndH1('Title Words Here', 'Header One Here');

      let reqs = spec.buildRequests([makeBid()], makeBidderRequest());
      expect(reqs[0].data.keywords.tokens).to.deep.equal(['json', 'ld', 'win']);

      // Remove JSON-LD → meta wins
      resetDom();
      setMetaKeywords('meta, wins');
      setTitleAndH1('Title Words', 'Header Words');
      reqs = spec.buildRequests([makeBid()], makeBidderRequest());
      expect(reqs[0].data.keywords.tokens).to.deep.equal(['meta', 'wins']);

      // Only RAKE fallback
      resetDom();
      setTitleAndH1('A Friendly Title About Dogs', 'How to Train a Dog');
      reqs = spec.buildRequests([makeBid()], makeBidderRequest());
      expect(reqs[0].data.keywords.tokens).to.be.an('array').that.is.not.empty;
    });

    it('detectKeywords: explicit params.keywords override ALL detection', function () {
      setJsonLd({ keywords: 'should,not,apply' });
      setMetaKeywords('also,ignored');

      const provided = ['custom1', 'custom2'];
      const reqs = spec.buildRequests([makeBid({ params: { keywords: provided } })], makeBidderRequest());
      const tokens = reqs[0].data.keywords.tokens;

      expect(tokens).to.deep.equal(provided);
    });

    it('detectKeywords: empty when no sources and barren page', function () {
      // No json-ld, no meta, blank title/h1 and root URL
      resetDom('https://host.only/');
      // no title / h1 set
      const reqs = spec.buildRequests([makeBid()], makeBidderRequest());
      const tokens = reqs[0].data.keywords.tokens;

      expect(tokens).to.be.an('array'); // may be [] depending on path; accept empty
    });
  });
});
