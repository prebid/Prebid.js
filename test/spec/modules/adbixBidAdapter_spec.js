...
describe('Adbix Bidder Adapter', function () {
  ...
  it('rejects a bid without publisherId', function () {
    const bid = { ...validBid, params: { placementId: 'test-300x250' } };
    expect(spec.isBidRequestValid(bid)).to.equal(false);
  });

  it('rejects a bid without placementId', function () {
    const bid = { ...validBid, params: { publisherId: 'test-publisher' } };
    expect(spec.isBidRequestValid(bid)).to.equal(false);
  });
  ...
  it('builds an OpenRTB-style test request for the Adbix endpoint', function () {
    ...
    expect(body.imp[0].ext.prebid.bidder.adbix.publisherId).to.equal('test-publisher');
    expect(body.imp[0].ext.prebid.bidder.adbix.placementId).to.equal('test-300x250');
  });
  ...
  it('parses a valid Adbix OpenRTB bid response', function () {
    ...
    expect(bids[0].meta.advertiserDomains).to.deep.equal(['adbix.net']);
  });
});
