...
describe('pigeoonBidAdapter', function() {
    ...
    describe('buildRequests', function() {
        ...
        it('should set gdpr to 1 when consent exists', function() {
        ...
        it('should include user id from cookie if available', function() {
            document.cookie = 'pigeoon_uid=test-user-id';
            const request = spec.buildRequests([validBid], bidderRequest);
            const data = JSON.parse(request.data);
            expect(data.user.id).to.equal('test-user-id');
        });

        it('should include native in imp when native media type exists', function() {
            const nativeBid = {
                ...validBid,
                mediaTypes: {
                    native: {
            ...
            const request = spec.buildRequests([nativeBid], bidderRequest);
            const data = JSON.parse(request.data);
    ...
    describe('getUserSyncs', function() {
        ...
        it('should return iframe sync url with gdpr consent', function() {
            const gdprConsent = {
                consentString: 'BOEFEAyOEFEAyAHABDENAI4AAAB9vABAASA',
                gdprApplies: true
            };
