...
import {spec} from 'modules/windtalkerBidAdapter';
import {getTopWindowLocation} from 'src/utils';
import {newBidder} from 'src/adapters/bidderFactory';

describe('Windtalker Adapter Tests', function () {
  const slotConfigs = [{
  ...
  const appSlotConfig = [{
    ...
    params: {
      ...
      app: {
        id: '1111',
        name: 'app name',
        bundle: 'io.windtalker.apps',
        storeUrl: 'http://windtalker.io/',
        domain: 'windtalker.io'
      }
    }
  }];
  ...
  it('Verify build request', function () {
    ...
    expect(request.url).to.equal('//windtalkerdisplay.hb.adp3.net/');
    ...
    expect(ortbRequest.site.publisher.id).to.equal('206768');
  ...
  it('Verify Video request', function () {
    ...
    expect(videoRequest.site.publisher.id).to.equal('206768');
  ...
  it('Verifies bidder code', function () {
    expect(spec.code).to.equal('windtalker');
  });
  ...
  it('Verify app requests', function () {
    ...
    expect(ortbRequest.app.publisher.id).to.equal('206768');
    expect(ortbRequest.app.id).to.equal('1111');
    expect(ortbRequest.app.name).to.equal('app name');
    expect(ortbRequest.app.bundle).to.equal('io.windtalker.apps');
    expect(ortbRequest.app.storeurl).to.equal('http://windtalker.io/apps');
    expect(ortbRequest.app.domain).to.equal('windtalker.io');
  });
