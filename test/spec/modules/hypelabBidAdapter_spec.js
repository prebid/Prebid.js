...
describe('hypelabBidAdapter', function () {
  ...
  describe('buildRequests', function () {
    ...
    it('should send the POST payload as text/plain', function () {
    ...
    it('should preserve an existing ORTB user.id', function () {
      const bidRequestWithEids = {
        ...mockValidBidRequest,
        userIdAsEids: [
          {
      ...
      const result = spec.buildRequests(
        [bidRequestWithEids],
        bidderRequestWithUser
      );
      expect(result[0].data.user.id).to.equal('first-party-id');
      expect(result[0].data.user.buyeruid).to.equal('pubcid_id');
    });
    ...
    it('should use an existing ORTB user.id instead of a temporary uuid', function () {
      const bidderRequestWithUser = {
        ...mockBidderRequest,
        ortb2: {
          ...mockBidderRequest.ortb2,
      ...
      expect(result[0].data.user.id).to.equal('first-party-id');
      expect(result[0].data.user.buyeruid).to.equal('first-party-id');
    });

    it('should include params.bidFloor when no dynamic floor is available', function () {
      const bidRequestWithFloor = {
        ...mockValidBidRequest,
        params: {
          ...mockValidBidRequest.params,
          bidFloor: 0.75,
          bidFloorCur: 'EUR',
        },
      };
      ...
      const result = spec.buildRequests(
        [bidRequestWithFloor],
        { ...mockBidderRequest, bids: [bidRequestWithFloor] }
      );
      expect(result[0].data.imp[0].bidfloor).to.equal(0.75);
      expect(result[0].data.imp[0].bidfloorcur).to.equal('EUR');
    });
