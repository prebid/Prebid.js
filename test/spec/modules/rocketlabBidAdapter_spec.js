...
import { spec } from "../../../modules/rocketlabBidAdapter.js";
import { BANNER, VIDEO, NATIVE } from "../../../src/mediaTypes.js";
import { getUniqueIdentifierStr } from "../../../src/utils.js";

const bidder = "rocketlab";

describe("RocketLabBidAdapter", function () {
  const userIdAsEids = [
    {
      source: "test.org",
      uids: [
  ...
  describe("buildRequests", function () {
    ...
    it("Returns valid placements", function () {
      ...
      for (let i = 0, len = placements.length; i < len; i++) {
        ...
        expect(placement.placementId).to.be.oneOf([
          "testBanner",
          "testVideo",
          "testNative",
        ]);
    ...
    it("Returns valid endpoints", function () {
      ...
      for (let i = 0, len = placements.length; i < len; i++) {
        const placement = placements[i];
        expect(placement.endpointId).to.be.oneOf([
          "testBanner",
          "testVideo",
          "testNative",
        ]);
    ...
    it("Returns data with gdprConsent and without uspConsent", function () {
      ...
      expect(data.gdpr.consentString).to.equal(
  ...
  describe("interpretResponse", function () {
    it("Should interpret banner response", function () {
      ...
      expect(dataItem.meta)
        .to.be.an("object")
        .that.has.any.key("advertiserDomains");
    });
    ...
    it("Should interpret native response", function () {
      ...
      expect(dataItem.native).to.have.keys(
        "clickUrl",
        "impressionTrackers",
        "title",
        "image"
      ...
      expect(dataItem.native.impressionTrackers).to.be.an("array").that.is.not
        .empty;
  ...
  describe("getUserSyncs", function () {
    it("Should return array of objects with proper sync config , include GDPR", function () {
      ...
      expect(syncData[0].url).to.equal(
        "https://usync.rocketlab.ai/image?pbjs=1&gdpr=1&gdpr_consent=ALL&coppa=0"
      );
    });
    it("Should return array of objects with proper sync config , include CCPA", function () {
      ...
      expect(syncData[0].url).to.be.a("string");
      expect(syncData[0].url).to.equal(
        "https://usync.rocketlab.ai/image?pbjs=1&ccpa_consent=1---&coppa=0"
      );
    });
    it("Should return array of objects with proper sync config , include GPP", function () {
      ...
      expect(syncData[0].url).to.be.a("string");
      expect(syncData[0].url).to.equal(
        "https://usync.rocketlab.ai/image?pbjs=1&gpp=abc123&gpp_sid=8&coppa=0"
      );
    });
  });
});
