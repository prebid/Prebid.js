...
// [Fix] Import from .ts instead of .js to match the TypeScript adapter file.
import { spec } from 'modules/billow_rtb25BidAdapter.ts';
  ...
  return Object.assign(
    {
      ...
      refererInfo: { page: 'https://example.test/page' },
      ortb2,
    },
    overrides
  );
...
describe('billow_rtb25BidAdapter', function () {
  ...
  describe('buildRequests', function () {
    it('should POST JSON to default endpoint', function () {
      ...
      // [Fix] Removed assertion for out.options (contentType: 'application/json').
      // The adapter relies on Prebid core's default POST behavior (text/plain + JSON body)
      // which avoids triggering CORS preflight. No options override is needed.
    ...
    it('should set imp.tagid from placementId and imp.id from bidId', function () {
      const bid = makeBannerBid({
        bidId: 'prebid-bid-xyz',
        params: { placementId: 'tag-from-plc' },
      });
      const out = spec.buildRequests([bid], makeBidderRequest());
    ...
    it('should emit one imp per bid (multi-slot)', function () {
      const bids = [
        makeBannerBid({ bidId: 'b1', params: { placementId: 'slot-a' } }),
        makeBannerBid({ bidId: 'b2', params: { placementId: 'slot-b' } }),
      ];
    ...
    /**
     * The adapter does not implement custom GDPR/CCPA/COPPA logic.
     * This test verifies that ortb2 privacy fields pass through to the ORTB payload.
     */
  ...
  describe('interpretResponse', function () {
    ...
    it('should map a banner OpenRTB bid to Prebid banner fields', function () {
      ...
      const body = {
        ...
        seatbid: [
          {
            bid: [
              openrtbBid('imp-match-1', '<div id="creative">billow</div>'),
            ],
          },
        ],
      };
    ...
    it('should replace ${AUCTION_PRICE} and ${AUCTION_LOSS} in banner ad markup', function () {
      ...
      const body = {
        id: 'resp-m',
        cur: 'USD',
        seatbid: [{ bid: [openrtbBid('imp-macros', adm)] }],
      };
    ...
    it('should replace all supported ${AUCTION_*} macros in banner ad markup', function () {
      ...
      const adm =
        '<div>https://track.example/imp?req=${AUCTION_ID}&bid=${AUCTION_BID_ID}&imp=${AUCTION_IMP_ID}' +
        '&seat=${AUCTION_SEAT_ID}&adid=${AUCTION_AD_ID}&p=${AUCTION_PRICE}&cur=${AUCTION_CURRENCY}' +
        '&mbr=${AUCTION_MBR}&loss=${AUCTION_LOSS}</div>';
      const body = {
        id: 'resp-all-macros',
        // Wrong cur on wire: adapter still exposes USD for bid + macros.
        cur: 'JPY',
        seatbid: [
          {
            seat: 'seat-42',
            bid: [openrtbBid('imp-all-macros', adm, { id: ortbBidId, adid: 'backend-adid-99' })],
          },
        ],
      };
    ...
    // When FEATURES.VIDEO is disabled (test-all-features-disabled), adapter falls back to banner.
    if (FEATURES.VIDEO) {
      it('should map a video OpenRTB bid adm to vastXml', function () {
        ...
        const body = {
          id: 'resp-v',
          cur: 'USD',
          seatbid: [{ bid: [openrtbBid('v-imp-1', vast)] }],
        };
      ...
      it('should replace auction macros inside vastXml', function () {
        const vast = '<?xml version="1.0"?><VAST version="3.0"><Ad><InLine>' +
          '<Impression><![CDATA[https://track.example/imp?p=${AUCTION_PRICE}&loss=${AUCTION_LOSS}]]></Impression>' +
          '</InLine></Ad></VAST>';
        const bid = makeVideoBid({ bidId: 'v-macro-1' });
        const body = {
          id: 'resp-vm',
          cur: 'USD',
          seatbid: [{ bid: [openrtbBid('v-macro-1', vast)] }],
        };
        const result = buildRequestThenInterpret([bid], makeBidderRequest(), body);
    ...
    if (FEATURES.NATIVE) {
      it('should map a native OpenRTB bid adm JSON to native.ortb', function () {
        ...
        const nativePayload = {
          assets: [{ id: 1, title: { text: 'Hello native' } }],
          link: { url: 'https://example.test/click' },
        };
        const body = {
          id: 'resp-n',
          cur: 'USD',
...
/*
 * Run locally:
 *   npx eslint "modules/billow_rtb25BidAdapter.ts" "test/spec/modules/billow_rtb25BidAdapter_spec.js" --cache --cache-strategy content
