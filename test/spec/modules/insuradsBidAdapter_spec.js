...
describe('InsurAds bid adapter tests', () => {
  ...
  describe('onBidWon()', () => {
    it('should log bid won information', () => {
      const logInfoSpy = sinon.spy();
      const originalLogInfo = spec.onBidWon.toString().includes('logInfo');
      expect(originalLogInfo).to.be.true;
      ...
      // Should not throw
      expect(() => spec.onBidWon(winningBid)).to.not.throw();
    });

    it('should not log if bidder does not match', () => {
      const winningBid = {
        bidder: 'other-bidder',
        adUnitCode: 'test-ad-unit',
        cpm: 5.0,
  ...
  describe('onAdRenderSucceeded()', () => {
    it('should log ad render information', () => {
      const originalLogInfo = spec.onAdRenderSucceeded.toString().includes('logInfo');
      expect(originalLogInfo).to.be.true;
      ...
      // Should not throw
      expect(() => spec.onAdRenderSucceeded(renderedBid)).to.not.throw();
    });
