...
describe('invibesBidAdapter:', function () {
  ...
  describe('TCF 2.3 disclosedVendors gating', function () {
    const baseVendorData = {
      gdprApplies: true,
      hasGlobalConsent: false,
      // vendor consent/LI granted for invibes
      vendor: {
        consents: { 436: true },
        legitimateInterests: { 436: true }
      },
      purpose: {
        consents: {
    ...
    it('keeps legacy behavior when disclosedVendors is missing (consent true => oi=2)', function () {
      const bidderRequest = {
        gdprConsent: { vendorData: JSON.parse(JSON.stringify(baseVendorData)) },
        refererInfo: { page: bidderRequestWithPageInfo.refererInfo.page }
      };

      const req = spec.buildRequests(bidRequests, bidderRequest);
      expect(req.data.oi).to.equal(2);
    });

    it('keeps legacy behavior when disclosedVendors exists but is empty (consent true => oi=2)', function () {
      const vendorData = JSON.parse(JSON.stringify(baseVendorData));
      vendorData.vendors = { disclosedVendors: {} }; // "implemented but empty" => treat as not implemented per adapter logic
    ...
    it('denies when disclosedVendors exists and our vendor is explicitly NOT disclosed (even if consent/LI true => oi=0)', function () {
      const vendorData = JSON.parse(JSON.stringify(baseVendorData));
      vendorData.vendors = { disclosedVendors: { 436: false } };
      ...
      const req = spec.buildRequests(bidRequests, bidderRequest);
      expect(req.data.oi).to.equal(0);
    });

    it('allows when disclosedVendors exists and our vendor is disclosed (consent true => oi=2)', function () {
      const vendorData = JSON.parse(JSON.stringify(baseVendorData));
      vendorData.vendors = { disclosedVendors: { 436: true } };
    ...
    it('denies when disclosedVendors exists and our vendor is NOT disclosed, even if LI-only would have allowed (oi=0)', function () {
      const vendorData = JSON.parse(JSON.stringify(baseVendorData));
      vendorData.vendor.consents[436] = false;
      vendorData.vendor.legitimateInterests[436] = true; // LI-only case
      vendorData.vendors = { disclosedVendors: { 436: false } };
    ...
    it('supports alternate disclosedVendors nesting under vendor (vendor.disclosedVendors)', function () {
      const vendorData = JSON.parse(JSON.stringify(baseVendorData));
      vendorData.vendor.disclosedVendors = { 436: false }; // your getter checks this path too
    ...
    it('supports rare flattened disclosedVendors (vendorData.disclosedVendors)', function () {
      const vendorData = JSON.parse(JSON.stringify(baseVendorData));
      vendorData.disclosedVendors = { 436: false }; // your getter checks this path too
