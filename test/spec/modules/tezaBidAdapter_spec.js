...
describe('tezaBidAdapter', function () {
  const bidderRequest = {
    auctionId: 'auc-1',
    timeout: 1200,
    refererInfo: { domain: 'localhost', page: 'http://localhost/test-teza.html' },
    gdprConsent: { gdprApplies: true, consentString: 'CONSENT' },
    uspConsent: '1YNN',
    gppConsent: { gppString: 'GPPSTRING', applicableSections: [7] },
    ortb2: { user: { buyeruid: 'u1' }, device: { dnt: 0 }, site: { cat: ['IAB1'] } }
  };
  ...
  describe('isBidRequestValid', () => {
    it('returns true when tagid and account exist', () => {
      expect(spec.isBidRequestValid(validBid)).to.equal(true);
    });
    it('returns false when missing account', () => {
  ...
  describe('buildRequests', () => {
    it('builds POST with account & test on query and expected ORTB fields', () => {
      const req = spec.buildRequests([validBid], bidderRequest);
      expect(req.method).to.equal('POST');
      expect(req.url).to.match(/^\/teza-auction\?test=1&account=acct123$/);
      ...
      expect(ortb.imp[0].banner.format).to.deep.equal([{ w: 300, h: 250 }, { w: 320, h: 50 }]);
  ...
  describe('interpretResponse', () => {
    it('maps OpenRTB seatbid to Prebid bids', () => {
  ...
  describe('onBidWon', () => {
    ...
    beforeEach(() => {
      ...
      global.Image = class {
        ...
        set src(url) { this._src = url; fired.push(url); }
      };
    });

    afterEach(() => { global.Image = OriginalImage; });

    it('pings burl (falls back to nurl) with cleared AUCTION_PRICE macro', () => {
      const bid = { burl: 'https://dsp/beacon?price=${AUCTION_PRICE}', nurl: 'https://dsp/win?price=${AUCTION_PRICE}', cpm: 1.23 };
      spec.onBidWon(bid);
      expect(fired).to.have.length(1);
      expect(fired[0]).to.equal('https://dsp/beacon?price=1.23');
    });

    it('uses nurl when burl absent', () => {
      const bid = { nurl: 'https://dsp/win?price=${AUCTION_PRICE}', cpm: 0.5 };
      spec.onBidWon(bid);
      expect(fired[0]).to.equal('https://dsp/win?price=0.5');
    });

    it('does nothing when neither url is present', () => {
      const bid = { cpm: 0.5 };
      spec.onBidWon(bid);
