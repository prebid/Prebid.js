...
describe('oftmedia RTD Submodule', function () {
  ...
  beforeEach(function () {
    ...
    sandbox.stub(userAgentUtils, 'getDeviceType').returns(1); sandbox.stub(userAgentUtils, 'getOS').returns('iOS'); sandbox.stub(userAgentUtils, 'getBrowser').returns('Safari');
  ...
  describe('Helper functions', function () {
    it('should create a timeout promise that resolves after specified time', function (done) {
      ...
      racePromise.then(result => {
        expect(promiseResolved).to.be.false; expect(result).to.be.undefined;
        done();
      }).catch(done);
    ...
    it('should create a timeout promise that resolves with undefined after specified time', function (done) {
      ...
      clock.tick(100); expect(resolved).to.be.false;
    ...
    it('should calculate remaining time correctly', function () {
      ...
      const startTime = Date.now() - 300; const maxDelay = 1000;
    ...
    it('should convert device types for specific bidders', function () {
      ...
      expect(convertDeviceTypeForBidder(0, 'oftmedia')).to.equal(2); expect(convertDeviceTypeForBidder(1, 'oftmedia')).to.equal(4); expect(convertDeviceTypeForBidder(2, 'oftmedia')).to.equal(5);
      ...
      expect(convertDeviceTypeForBidder("1", 'oftmedia')).to.equal(4); expect(convertDeviceTypeForBidder("1", 'appnexus')).to.equal(4); expect(convertDeviceTypeForBidder("1", 'pubmatic')).to.equal("1");
    });
  });
  ...
  describe('Script loading functionality', function () {
    ...
    it('should reject when publisher ID is missing', function (done) {
      ...
      const scriptPromise = loadOftmediaScript({ params: {} });
  ...
  describe('ORTB2 data building', function () {
    it('should build valid ORTB2 data object with device and keywords', function () {
      ...
      expect(result.ortb2Data.device.devicetype).to.equal(4); expect(result.ortb2Data.device.os).to.equal('iOS');
      ...
      expect(result.ortb2Data.site.keywords).to.include('deviceBrowser=Safari');
    });
  ...
  describe('Bid request processing', function () {
    it('should process bid request and enrich it with ORTB2 data', function (done) {
      ...
      oftmediaRtd.oftmediaRtdSubmodule.getBidRequestData(bidConfig, () => {
        ...
        expect(bidConfig.ortb2Fragments.bidder.appnexus.device.devicetype).to.equal(4); expect(bidConfig.ortb2Fragments.bidder.appnexus.device.os).to.equal('iOS');
        expect(bidConfig.ortb2Fragments.bidder.appnexus.site.keywords).to.include('deviceBrowser=Safari');
        done();
    ...
    it('should enrich bid request with keywords, OS and device when enrichRequest is true', function (done) {
      ...
      oftmediaRtd.oftmediaRtdSubmodule.getBidRequestData(bidConfig, function (error) {
        ...
        try {
          ...
          expect(bidConfig.ortb2Fragments.bidder.appnexus.device.os).to.equal('iOS');
          done();
        } catch (e) {
          done(e);
        }
