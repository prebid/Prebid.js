...
describe('hiromedia adapter', function() {
  ...
  describe('callbids', () => {
    ...
    it('invokes a single bid request for a single valid placement', () => {
      ...
      // callback
      expect(query).to.have
        .property('callback')
        .and.to.equal('$$PREBID_GLOBAL$$.' + DEFAULT_CALLBACK_NAME);

      // batch key
      expect(query).to.have
        .property('batchKey')
        .and.to.equal([DEFAULT_ENDPOINT, '1337', '300x250'].join('-'));

      // placementCode
      expect(query).to.have
        .property('placementCode')
        .and.to.equal('div-gpt-ad-12345-1');
      ...
      expect(query).to.have
        .property('domain')
        .and.to.equal(window.top.location.hostname);
    });
    ...
    it('finds an allowed size in a placement with multiple sizes', () => {
      ...
      expect(query).to.have
        .property('placementSizes')
        .and.to.equal('300x600,300x250');
    });
  ...
  describe('global response handler', () => {
    ...
    const getResponseHandler = () =>
      window.$$PREBID_GLOBAL$$[DEFAULT_CALLBACK_NAME];
    ...
    it('adds a bid reponse for each pending bid', () => {
      ...
      expect(bidObject).to.have
        .property('ad')
        .and.to.equal('<script src="ad.js"></script>');
      ...
      expect(secondBidObject).to.have
        .property('ad')
        .and.to.equal('<script src="ad.js"></script>');
