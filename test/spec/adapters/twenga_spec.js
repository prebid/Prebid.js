describe('twenga adapter tests', function() {
  ...
  it('sets url parameters', function() {
    ...
    expect(parsedBidUrlQueryString).to.have
      .property('callback')
      .and.to.equal('$$PREBID_GLOBAL$$.handleTwCB');
    expect(parsedBidUrlQueryString).to.have
      .property('callback_uid')
      .and.to.equal('tw_abcd1234');
  ...
  it('creates a bid response if bid is returned', function() {
    var stubLoadScript = sinon.stub(adLoader, 'loadScript', function(url) {
      ...
      $$PREBID_GLOBAL$$._bidsRequested.push({
        bidderCode: DEFAULT_PARAMS.bidderCode,
        bids: [
          {
    ...
    expect(bidResponseAd).to.have
      .property('cpm')
      .and.to.equal(BID_RESPONSE.result.cpm / 10000);
    expect(bidResponseAd).to.have
      .property('adUrl')
      .and.to.equal(BID_RESPONSE.result.ad);
    expect(bidResponseAd).to.have
      .property('width')
      .and.to.equal(BID_RESPONSE.result.width);
    expect(bidResponseAd).to.have
      .property('height')
      .and.to.equal(BID_RESPONSE.result.height);
