...
describe('thoughtleadr adapter tests', function() {
  ...
  describe('callBids', function() {
    ...
    it("should request page.js from cdn if there wasn't before", function() {
      ...
      chai_1
        .expect(loadScript.getCall(0).args[0])
        .to.be.equal('//cdn.thoughtleadr.com/v4/page.js');
    });
    ...
    it('should use window.tldr.config.root_url', function() {
      ...
      chai_1
        .expect(loadScript.getCall(0).args[0])
        .to.be.equal('http://example.loc/page.js');
    });
  ...
  describe('handleBids', function() {
    ...
    it('should filter invalid bids', function() {
      ...
      chai_1
        .expect(requestPlacement.getCall(0).args[0])
        .to.be.equal(request.bids[1]);
    });
  });
  ...
  describe('requestPlacement', function() {
    ...
    it('should made request through page.js api', function() {
      ...
      chai_1
        .expect(tldrRequestPrebid.firstCall.args[0])
        .to.be.equal(request.bids[0].params.placementId);
      ...
      window.addEventListener(
        ...
        function(ev) {
          if (ev.data && ev.data.TLDR_RESPONSE) {
            ...
            chai_1
              .expect(JSON.stringify(ev.data.TLDR_RESPONSE.config))
              .to.be.equal('{"abc":567}');
            done();
