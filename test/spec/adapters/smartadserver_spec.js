describe('smartadserver adapter tests', function() {
  ...
  it('set url parameters', function() {
    ...
    expect(parsedBidUrlQueryString).to.have
      .property('pbjscbk')
      .and.to.equal('pbjs.' + smartCallback);
    expect(parsedBidUrlQueryString).to.have
      .property('siteid')
      .and.to.equal('1234');
    expect(parsedBidUrlQueryString).to.have
  ...
  it('creates a bid response if bid is returned', function() {
    ...
    expect(bidResponseAd).to.have
      .property('cpm')
      .and.to.equal(BID_RESPONSE.cpm);
    expect(bidResponseAd).to.have.property('ad').and.to.equal(BID_RESPONSE.ad);
    expect(bidResponseAd).to.have
      .property('width')
      .and.to.equal(BID_RESPONSE.width);
    expect(bidResponseAd).to.have
      .property('height')
      .and.to.equal(BID_RESPONSE.height);
