...
describe('indexExchange adapter - Request', function() {
  ...
  it('test_prebid_indexAdapter_parameter_x3: prebid sends AS request -> x3 parameter does not exist in the request', function() {
    ...
    assert.notInclude(
      ...
      'x3=',
      'x3 parameter is not in AS request'
    );
  });
  ...
  it('test_prebid_indexAdapter_request_6: threshold valid + non valid which exceeds threshold -> 1 Ad Server request with supported sizes only', function() {
    ...
    var invalidSlotPlacement = IndexUtils.DefaultPlacementCodePrefix +
      'invalid';
    var invalidSlotID = 'slot-invalid';
    configuredBids.push(
      IndexUtils.createBidSlot(invalidSlotPlacement, invalidSlotID, [
        IndexUtils.unsupportedSizes[0]
      ])
    );
    ...
    assert.equal(
      sidMatched.unmatched.configured.length,
      unsupportedSizeCount,
      unsupportedSizeCount + ' of configured bids is missing in impression Obj'
    );
    assert.equal(
      sidMatched.unmatched.configured[0].placementCode,
      invalidSlotPlacement,
      "missing slot's placement code is " + invalidSlotPlacement
    );
    assert.equal(
      sidMatched.unmatched.configured[0].params.id,
      invalidSlotID,
      "missing slot's slotID is " + invalidSlotID
    );
  ...
  it('test_prebid_indexAdapter_request_7: multiple sizes with slots that exceeds max threshold requests -> 1 Ad Server request with supported sizes only', function() {
    ...
    assert.equal(
      ...
      requestSlotNumber * requestSizeNumber,
      'All slots each with multiple sizes are in AS request'
    );
    for (var i = 0; i < sidMatched.matched.length; i++) {
      var pair = sidMatched.matched[i];
      assert.equal(
  ...
  it('test_prebid_indexAdapter_request_sizeID_1: 1 prebid size slot, 1 index slot with size -> one slot in AS request 1 no size ID', function() {
    ...
    assert.equal(
      ...
      slotSizes[0],
      'the width made in the request matches with request: ' + slotSizes[0]
    );
    assert.equal(
      impressionObj[0].banner.h,
      slotSizes[1],
      'the height made in the request matches with request: ' + slotSizes[1]
    );
    assert.equal(
      impressionObj[0].ext.sid,
      slotID,
      'slotID in the request matches with configuration: ' + slotID
    );
    assert.equal(
  ...
  it('test_prebid_indexAdapter_request_sizeID_2: multiple prebid size slot, 1 index slot with size -> one slot in AS request 1 no size ID', function() {
    ...
    var configuredBids = [
      IndexUtils.createBidSlot(
        IndexUtils.DefaultPlacementCodePrefix,
        slotID,
        [slotSizes, IndexUtils.supportedSizes[1]],
        { slotSize: slotSizes }
      )
    ];
  ...
  it('test_prebid_indexAdapter_request_sizeID_3: multiple prebid size slot, index slots with size for all prebid slots -> all size in AS request, no size ID', function() {
    ...
    assert.equal(
      impressionObj[0].banner.w,
      slotSizes_1[0],
      'the width made in the request matches with request: ' + slotSizes_1[0]
    );
    assert.equal(
      impressionObj[0].banner.h,
      slotSizes_1[1],
      'the height made in the request matches with request: ' + slotSizes_1[1]
    );
    assert.equal(
      impressionObj[0].ext.sid,
      slotID_1,
      'slotID in the request matches with configuration: ' + slotID_1
    );
    assert.equal(
      impressionObj[0].ext.siteID,
      IndexUtils.DefaultSiteID,
