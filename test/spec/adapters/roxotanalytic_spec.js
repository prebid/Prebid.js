describe('Roxot adapter tests', function(){
  ...
  const adapter = require('src/adapters/roxotanalytic');
  ...
  describe('roxotResponseHandler', function () {
    ...
    it('should add empty bid responses if no bids returned', function () {
      ...
      var bidderRequest = {
        bidderCode: 'roxotanalytic',
        bids: [
          {
            bidId: 'id1',
            bidder: 'roxotanalytic',
            sizes: [[320, 50]],
      ...
      expect(bidObject1.bidderCode).to.equal('roxotanalytic');

      expect(bidPlacementCode2).to.equal('div-gpt-ad-12345-2');
      expect(bidObject2.getStatusCode()).to.equal(2);
      expect(bidObject2.bidderCode).to.equal('roxotanalytic');
