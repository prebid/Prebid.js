...
describe('placementPositionInfo', function () {
  ...
  describe('getPlacementInfo', function () {
    ...
    beforeEach(function () {
      ...
      sandbox.stub(document, 'getElementById').returns(mockElement);
      ...
      const utils = getPlacementPositionUtils();
      getPlacementInfo = utils.getPlacementInfo;
    });
  ...
  describe('getPlacementEnv', function () {
    ...
    beforeEach(function () {
      ...
      visibilityStateStub = sandbox.stub(document, 'visibilityState').value('visible');

      sandbox.stub(document, 'body').value({
        scrollHeight: 2000,
        offsetHeight: 1800
      });

      sandbox.stub(document, 'documentElement').value({
        clientHeight: 1900,
        scrollHeight: 2100,
        offsetHeight: 1950
      });
      ...
      const utils = getPlacementPositionUtils();
      getPlacementEnv = utils.getPlacementEnv;
    });
    ...
    it('should return TimeFromNavigation as floored performance.now()', function () {
      ...
      const utils = getPlacementPositionUtils();
      const result = utils.getPlacementEnv();
    ...
    it('should return TabActive as false when document is hidden', function () {
      ...
      sandbox.stub(utils, 'getWindowTop').returns(mockWindow);
      ...
      sandbox.stub(document, 'body').value({ scrollHeight: 1000, offsetHeight: 1000 });
      sandbox.stub(document, 'documentElement').value({
        clientHeight: 1000, scrollHeight: 1000, offsetHeight: 1000
      });
  ...
  describe('getViewableDistance edge cases', function () {
    ...
    beforeEach(function () {
      sandbox.stub(document, 'getElementById').returns({ id: 'test' });
