...
describe(`
    ...
    describe(`SCENARIO: The client side auction is performed to select the winning bid.
      ...
      describe(`WHEN: The auction is complete by all bids being available or by timeout`, () => {
        ...
        it(`THEN: AOL Analytics track is called for the auction complete event`, () => {
          assert.ok(spyTrack.calledWith({eventType: AUCTION_COMPLETED,
    ...
    describe(`SCENARIO: The Ad Server side auction is performed, bid is won and the ad is rendedered.
      ...
      it(`THEN: AOL Analytics track is called for the bid won event`, () => {
        assert.ok(spyTrack.calledWith({eventType: BID_WON,
          args: bidWon}));
      });
