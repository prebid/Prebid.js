...
exports.createBidSlots = function(numSlot, numSize) {
  ...
  var mkPlacementCode = function(i, j) {
    return DefaultPlacementCodePrefix + i + '_' + j;
  };
  for (var i = 0; i < bids.length; i++) {
    var requestSizes = new Array(numSize);
    for (var j = 0; j < requestSizes.length; j++)
...
exports.matchOnPlacementCode = function(expected, prebid) {
  ...
  return {
    unmatched: { expected: compared.lhsOnly, prebid: compared.rhsOnly },
    matched: compared.intersection.map(function(pair) {
      return {
        expected: pair.left,
        prebid: pair.right,
...
exports.matchBidsOnSID = function(lhs, rhs) {
  ...
  var matched = compared.intersection.map(function(pair) {
    return { configured: pair.left, sent: pair.right, name: pair.name };
  });
...
exports.getBidResponse = function(
  configuredBids,
  urlJSON,
  optionalPriceLevel,
  ...
  for (var i = 0; i < configuredBids.length; i++) {
    ...
    for (var j = 0; j < sizes.length; j++) {
      if (
        typeof optionalPassOnBid[i] !== 'undefined' &&
        typeof optionalPassOnBid[i][j] !== 'undefined' &&
        optionalPassOnBid[i][j]
      )
        continue;
      ...
      if (typeof optionalResponseIdentifier !== 'undefined')
        bid.adm += '_' + optionalResponseIdentifier;
      bid.ext = {};
      ...
      if (
        typeof optionalResponseParam !== 'undefined' &&
        typeof optionalResponseParam[i] !== 'undefined' &&
        typeof optionalResponseParam[i][j] !== 'undefined'
      ) {
        optionalSlotParam = optionalResponseParam[i][j];
      }

      if (
        typeof optionalSlotParam !== 'undefined' &&
        typeof optionalSlotParam.ext !== 'undefined' &&
        optionalSlotParam.ext.dealid !== 'undefined'
      ) {
      ...
      if (
        typeof optionalPriceLevel !== 'undefined' &&
        optionalPriceLevel[i].length !== 0
      ) {
  ...
  for (var m = 0; m < asAllBids.length; m++) {
    ...
    for (var i = 0; i < asBids.length; i++) {
      ...
      for (var j = 0; j < configuredBids.length; j++) {
        ...
        if (
          typeof asBids[i].ext !== 'undefined' &&
          typeof asBids[i].ext.dealid !== 'undefined'
        ) {
