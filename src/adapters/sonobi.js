...
var SonobiAdapter = function SonobiAdapter() {
  ...
  function _phone_in(request) {
    ...
    var ref = window.frameElement
      ? '&ref=' + encodeURI(top.location.host || document.referrer)
      : '';
    adloader.loadScript(
      trinity +
  ...
  function _keymaker(adSlots) {
    ...
    utils._each(adSlots, function(bidRequest) {
      if (bidRequest.params) {
        ...
        //  Mandatory
        var slotIdentifier = bidRequest.params.ad_unit
          ? bidRequest.params.ad_unit
          : bidRequest.params.placement_id
              ? bidRequest.params.placement_id
              : null;
        ...
        var args = sizes
          ? floor ? sizes + '|f=' + floor : sizes
          : floor ? 'f=' + floor : '';
        if (/^[\/]?[\d]+[[\/].+[\/]?]?$/.test(slotIdentifier)) {
          slotIdentifier = slotIdentifier.charAt(0) === '/'
            ? slotIdentifier
            : '/' + slotIdentifier;
          keyring[slotIdentifier + '|' + bidId] = args;
          keymakerAssoc[
            slotIdentifier + '|' + bidId
          ] = bidRequest.placementCode;
          bidReqAssoc[bidRequest.placementCode] = bidRequest;
        } else if (
          /^[0-9a-fA-F]{20}$/.test(slotIdentifier) &&
          slotIdentifier.length === 20
        ) {
