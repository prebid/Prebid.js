...
function WidespaceAdapter() {
  let useSSL = 'https:' === document.location.protocol,
    baseURL = (useSSL ? 'https:' : 'http:') +
      '//engine.widespace.com/map/engine/hb/dynamic?',
  ...
  function _callBids(params) {
    ...
    for (var i = 0; i < bids.length; i++) {
      ...
      sizeQueryString = parsedSizes.reduce(
        (prev, curr) => {
      ...
      // Expose the callback
      $$PREBID_GLOBAL$$.widespaceHandleCB = (window[
        callbackName
      ] = handleCallback);
  ...
  var handleCallback = function handleCallback(bidsArray) {
    ...
    var bidObject, bidCode = 'widespace';

    for (var i = 0, l = bidsArray.length; i < l; i++) {
      var bid = bidsArray[i], placementCode = '', validSizes = [];
      ...
      if (
        ...
        bid.callbackUid &&
        bid.status !== 'noad' &&
        verifySize(bid.sizes, validSizes)
      ) {
