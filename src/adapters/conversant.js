...
var ConversantAdapter = function() {
  ...
  var addEmptyBidResponses = function(placementsWithBidsBack) {
    ...
    if (allConversantBidRequests && allConversantBidRequests.bids) {
      utils._each(allConversantBidRequests.bids, function(conversantBid) {
        if (
          !utils.contains(placementsWithBidsBack, conversantBid.placementCode)
        ) {
  ...
  var parseSeatbid = function(bidResponse) {
    ...
    utils._each(bidResponse.bid, function(conversantBid) {
      ...
      var bidRequested = $$PREBID_GLOBAL$$._bidsRequested
        .find(bidSet => bidSet.bidderCode === 'conversant')
      ...
      if (bidRequested) {
        ...
        if (responseCPM !== 0.0) {
          ...
          if (
            sizeArrayLength === 2 &&
            typeof bidRequested.sizes[0] === 'number' &&
            typeof bidRequested.sizes[1] === 'number'
          ) {
  ...
  $$PREBID_GLOBAL$$.conversantResponse = function(conversantResponseObj, path) {
    ...
    if (conversantResponseObj && conversantResponseObj.id) {
      if (
        conversantResponseObj.seatbid &&
        conversantResponseObj.seatbid.length > 0 &&
        conversantResponseObj.seatbid[0].bid &&
        conversantResponseObj.seatbid[0].bid.length > 0
      ) {
