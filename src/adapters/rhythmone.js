...
module.exports = function(bidManager, global, loader) {
  ...
  var bidderCode = 'rhythmone', bidLostTimeout = null;
  ...
  function getRMPURL(bidParams, ortbJSON, bids) {
    ...
    if (bidParams.method === 'get') {
      ...
      var placementCodes = [], heights = [], widths = [], floors = [];
  ...
  this.callBids = function(params) {
    var slotMap = {}, bidParams = getBidParameters(params.bids);
    ...
    load(
      bidParams,
      getRMPURL(bidParams, ortbJSON, params.bids),
      JSON.stringify(ortbJSON),
      function(code, msg, txt) {
        ...
        if (code === -1)
          ...
          try {
            ...
            for (var i = 0; result.seatbid && i < result.seatbid.length; i++)
              for (
                var j = 0;
                result.seatbid[i].bid && j < result.seatbid[i].bid.length;
                j++
              )
