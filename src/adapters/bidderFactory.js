 ...
 */
export const processBidderRequests = hook('async', function processBidderRequests(spec, bids, bidderRequest, ajax, wrapCallback, {onRequest, onResponse, onPaapi, onError, onBid, onCompletion}) {
  ...
  return postBuildRequests(spec, bids, bidderRequest, requests)
    .then((requests) => {
      ...
      logError(`Error calling buildRequests on '${spec.code}'`, err)
    });
