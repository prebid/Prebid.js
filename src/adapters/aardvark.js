...
var AARDVARK_CALLBACK_NAME = 'aardvarkResponse',
  ...
  requestBids = function(bidderCode, callbackName, bidReqs) {
    var ref = utils.getTopWindowLocation(), ai = '', scs = [], bidIds = [];

    ref = ref ? ref.host : DEFAULT_REFERRER;

    for (
      var i = 0, l = bidReqs.length, bid, _ai, _sc, _endpoint;
      i < l;
      i += 1
    ) {
      bid = bidReqs[i];
    ...
    if (!ai.length || !scs.length)
      return utils.logWarn(
        'Bad bid request params given for adapter $' +
          bidderCode +
          ' (' +
  ...
  registerBidResponse = function(bidderCode, rawBidResponse) {
    ...
    bidResponse.ad = rawBidResponse.adm +
      utils.createTrackPixelHtml(decodeURIComponent(rawBidResponse.nurl));
  ...
  registerAardvarkCallback = function(bidderCode, callbackName) {
    $$PREBID_GLOBAL$$[callbackName] = function(rtkResponseObj) {
      ...
      for (var bidRequestId in $$PREBID_GLOBAL$$[AARDVARK_REQUESTS_MAP][
        bidderCode
      ])
        if (
  ...
  AardvarkAdapter = function() {
    ...
    $$PREBID_GLOBAL$$[AARDVARK_REQUESTS_MAP] = $$PREBID_GLOBAL$$[
      AARDVARK_REQUESTS_MAP
    ] || {};
