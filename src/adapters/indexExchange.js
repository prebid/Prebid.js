...
window.cygnus_index_parse_res = function(response) {
  try {
    if (response) {
      if (
        typeof _IndexRequestData !== 'object' ||
        typeof _IndexRequestData.impIDToSlotID !== 'object' ||
        typeof _IndexRequestData.impIDToSlotID[response.id] === 'undefined'
      ) {
        return;
      }
      var targetMode = 1;
      var callbackFn;
      if (
        typeof _IndexRequestData.reqOptions === 'object' &&
        typeof _IndexRequestData.reqOptions[response.id] === 'object'
      ) {
        if (
          typeof _IndexRequestData.reqOptions[response.id].callback ===
          'function'
        ) {
          callbackFn = _IndexRequestData.reqOptions[response.id].callback;
        }
        if (
          typeof _IndexRequestData.reqOptions[response.id].targetMode ===
          'number'
        ) {
      ...
      var allBids = [];
      var seatbidLength = typeof response.seatbid === 'undefined'
        ? 0
        : response.seatbid.length;
      for (var i = 0; i < seatbidLength; i++) {
        for (var j = 0; j < response.seatbid[i].bid.length; j++) {
          var bid = response.seatbid[i].bid[j];
          if (
            typeof bid.ext !== 'object' ||
            typeof bid.ext.pricelevel !== 'string'
          ) {
            continue;
          }
          if (
            typeof _IndexRequestData.impIDToSlotID[response.id][bid.impid] ===
            'undefined'
          ) {
            continue;
          }
...
var getParameterByName = function(name) {
  ...
  for (
    var x = 0;
    x < CONSTANTS.INDEX_DEBUG_MODE.mode.sandbox.topFrameLimit;
    x++
  ) {
    if (wdw.parent == wdw) {
      break;
...
var cygnus_index_start = function() {
  ...
  function OpenRTBRequest(siteID, parseFn, timeoutDelay) {
    ...
    if (typeof _IndexRequestData.requestCounter === 'undefined') {
      ...
      _IndexRequestData.requestCounter = (_IndexRequestData.requestCounter +
        1) %
        256;
    }
  ...
  OpenRTBRequest.prototype.setPageOverride = function(sitePageOverride) {
    if (
      typeof sitePageOverride === 'string' && !sitePageOverride.match(/^\s*$/)
    ) {
  ...
  OpenRTBRequest.prototype.addImpression = function(
    width,
    height,
    bidFloor,
    bidFloorCurrency,
    ...
    if (
      (typeof slotID === 'string' || typeof slotID === 'number') &&
    ...
    if (typeof bidFloor === 'number') {
      ...
      if (
        bidFloorCurrency !== undefined && typeof bidFloorCurrency !== 'string'
      ) {
        return null;
      }
  ...
  OpenRTBRequest.prototype.buildRequest = function() {
    ...
    if (
      getIndexDebugMode() ==
      CONSTANTS.INDEX_DEBUG_MODE.mode.sandbox.queryValue.toUpperCase()
    ) {
    ...
    scriptSrc += '/cygnus?v=7&fn=cygnus_index_parse_res&s=' +
      this.siteID +
      '&r=' +
      jsonURI +
      '&pid=pb' +
      prebidVersion;
    if (
      typeof this.timeoutDelay === 'number' &&
  ...
  try {
    if (
      typeof cygnus_index_args === 'undefined' ||
      typeof cygnus_index_args.siteID === 'undefined' ||
      typeof cygnus_index_args.slots === 'undefined'
    ) {
      return;
    }
...
var IndexExchangeAdapter = function IndexExchangeAdapter() {
  ...
  function _callBids(request) {
    ...
    if (
      !utils.hasValidBidRequest(bidArr[0].params, requiredParams, ADAPTER_NAME)
    ) {
      return;
    }
    ...
    for (var i = 0; i < bidArr.length; i++) {
      ...
      for (var j = 0; j < bid.sizes.length; j++) {
        ...
        for (var k = 0; k < cygnus_index_adunits.length; k++) {
          if (
            bid.sizes[j][0] == cygnus_index_adunits[k][0] &&
        ...
        if (bid.params.size && bid.params.size instanceof Array) {
          if (
            !(bid.sizes[j][0] == bid.params.size[0] &&
              bid.sizes[j][1] == bid.params.size[1])
          )
            continue;
        ...
        if (
          bid.params.timeout && typeof cygnus_index_args.timeout === 'undefined'
        ) {
        ...
        if (
          utils.hasValidBidRequest(bid.params, requiredParams, ADAPTER_NAME)
        ) {
        ...
        for (var adSlotId in slotIdMap) {
          ...
          for (var cpmAndSlotId in indexObj) {
            ...
            if (slotID === adSlotId) {
              ...
              if (
                typeof _IndexRequestData.targetIDToResp === 'object' &&
                typeof _IndexRequestData.targetIDToResp[cpmAndSlotId] ===
                  'object' &&
                typeof _IndexRequestData.targetIDToResp[cpmAndSlotId].dealID !==
                  'undefined'
              ) {
                if (
                  typeof _IndexRequestData.targetAggregate['private'][
                    adUnitCode
                  ] === 'undefined'
                )
                ...
                if (
                  typeof _IndexRequestData.targetAggregate['open'][
                    adUnitCode
                  ] === 'undefined'
                )
