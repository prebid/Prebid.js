  ...
  function _callBids(request) {
    ...
    for (var i = 0; i < bidArr.length; i++) {
      ...
      for (var j = 0; j < bid.sizes.length; j++) {
        ...
        // Check for size defined in bidder params (PBA-8) (defining siteID per size)
        if (bid.params.size && bid.params.size instanceof Array) {
          // Skip sizes that don't match our configured size
        ...
        if (utils.hasValidBidRequest(bid.params, requiredParams, ADAPTER_NAME)) {
          ...
          if ( typeof slotID !== 'string' && typeof slotID !== 'number' ){
            utils.logError("bid contains invalid slot ID from " + bid.placementCode + ". Discarding slot", ADAPTER_NAME);
            continue
          }
