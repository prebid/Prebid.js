...
function checkDefinedPlacement(id) {
  var placementCodes = $$PREBID_GLOBAL$$._bidsRequested
    .map(bidSet => bidSet.bids.map(bid => bid.placementCode))
    .reduce(flatten)
    .filter(uniques);
...
$$PREBID_GLOBAL$$.getAdserverTargeting = function(adUnitCode) {
  ...
  return targeting
    ...
    .reduce(
      function(accumulator, targeting) {
...
$$PREBID_GLOBAL$$.getBidResponses = function() {
  ...
  const currentRequestId = responses &&
    responses.length &&
    responses[responses.length - 1].requestId;
...
$$PREBID_GLOBAL$$.bidsAvailableForAdapter = function(bidderCode) {
  ...
  $$PREBID_GLOBAL$$._bidsRequested
    .find(bidderRequest => bidderRequest.bidderCode === bidderCode)
    .bids.map(bid => {
