...
1. The content page (`amp.publisher.com/page.html` in the diagram) is just a normal AMP page that serves over HTTPS
    ...
    + It uses the `amp-ad` element, which loads an iframe from a different domain (which is what we call the "cross-domain host" below).
...
4. The cross-domain host (`amp-x-domain.publisher.com/remote.html` in the diagram) runs a Prebid auction, fetching creatives for each of the ad slots defined in the content page. 
...
For more information, see [the AMP example in our Github repo](https://github.com/prebid/Prebid.js/tree/master/integrationExamples/gpt/amp) (which uses the same filenames as in the diagram below).

![Prebid AMP Overview Diagram]({{site.github.url}}/assets/images/dev-docs/prebid-amp.png)

## Related Topics

+ [Show Prebid Ads on AMP Pages]({{site.github.url}}/dev-docs/show-prebid-ads-on-amp-pages.html) (Developer implementation guide)
+ [Setting up Prebid for AMP in DFP]({{}}/adops/setting-up-prebid-for-amp-in-dfp.html) (Ad ops setup)
